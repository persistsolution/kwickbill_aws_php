...
if($_POST['action'] == 'fetch_record'){
 $id = $_POST['id'];
    $query = "SELECT * FROM tbl_popup_image WHERE id = '$id'";
...
$ModifiedDate = date('Y-m-d');
  $query2 = "UPDATE tbl_popup_image SET Status='$Status',Photo='$Photo',modifiedby='$user_id' WHERE id = '$id'";
 	$conn->query($query2);
  echo 1;
}
  ...
  if($_POST['action'] == 'delete') {
      ...
      $query = "DELETE FROM tbl_popup_image WHERE id = '$id'";
      $conn->query($query);
...
if($_POST['action'] == 'deletePhoto'){
    $id = $_POST['id'];
    $Photo = $_POST['Photo'];
        $q = "UPDATE tbl_popup_image SET Photo='' WHERE id=$id";
        $conn->query($q);
        $src = "../../uploads/$Photo";
        unlink($src);
...
<table id="example" class="table table-striped table-bordered dt-responsive nowrap" style="width:100%">
        ...
        <tbody>
          <?php 
 $srno = 1;
  $sql = "SELECT * FROM tbl_popup_image ORDER BY id DESC";
   $rx = $conn->query($sql);
 ...
 <?php }
   ...
   if($_POST['action'] == 'getProdDetails') {
    $id = $_POST['id'];
    $VedId = $_POST['VedId'];
    $sql = "SELECT * FROM tbl_popup_image WHERE id='$id'";
    $row = getRecord($sql);
    $Price = $row['Price'];
