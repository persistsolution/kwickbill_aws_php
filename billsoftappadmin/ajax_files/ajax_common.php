...
$qx = "INSERT INTO tbl_common_master SET Name = '$Name',Status='$Status',Roll='$Roll'";
	$conn->query($qx);
	echo 1;
...
$ModifiedDate = date('Y-m-d');
  ...
  $query2 = "UPDATE tbl_common_master SET Name = '$Name',Status='$Status',Roll='$Roll' WHERE id = '$id'";
 	$conn->query($query2);
...
<table id="example" class="table table-striped table-bordered dt-responsive nowrap" style="width:100%">
 ...
 $srno = 1;
 $Roll = $_POST['Roll'];
  $sql = "SELECT tc.* FROM tbl_common_master tc 
          WHERE tc.Roll='$Roll' ORDER BY tc.id DESC";
   $rx = $conn->query($sql);
