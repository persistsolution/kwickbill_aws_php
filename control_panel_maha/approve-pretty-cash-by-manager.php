...
$CreatedDate = date('Y-m-d');
 ...
 //$TicketNo= "#".rand(1000,9999);
     ...
     $query2 = "UPDATE tbl_prettycash_request SET MannagerApproveDate='$ApproveDate',MannagerComment='$MannagerComment',MannagerAmount='$MgrAmount',ManagerStatus='$ManagerStatus',MrgBy='$user_id' WHERE id = '$id'";
  $conn->query($query2);
...
if($_GET['page'] == 'ho'){
 echo "<script>alert('Approved Successfully!');window.location.href='ho-pending-pretty-cash-request.php';</script>";
  ...
  else if($_GET['page'] == 'adminabove'){
 echo "<script>alert('Approved Successfully!');window.location.href='admin-above-pending-pretty-cash-request.php';</script>";
  }
  else{
  echo "<script>alert('Approved Successfully!');window.location.href='manager-pending-pretty-cash-request.php';</script>";
