...
$id = $_GET['id'];
$CompId = $_GET['qid'];
$sql7 = "SELECT * FROM tbl_insurance_clain_details WHERE id='$id'";
$row7 = getRecord($sql7);

$sql77 = "SELECT * FROM tbl_insurance_clain WHERE id='$CompId'";
...
if($_GET['id']==''){
     $qx = "INSERT INTO tbl_insurance_clain_details SET CompId='$CompId',Message='$Message',ClainStatus='$ClainStatus',CreatedDate='$CreatedDate',CreatedBy='$user_id'";
  $conn->query($qx);

  $sql = "UPDATE tbl_insurance_clain SET ClainStatus='$ClainStatus' WHERE id='$CompId'";
  $conn->query($sql);

  echo "<script>alert('Insurance Claim Record Saved Successfully!');window.location.href='view-insurance-claim-action.php?id=$CompId';</script>";
}
else{
 
    $query2 = "UPDATE tbl_insurance_clain_details SET CompId='$CompId',Message='$Message',ClainStatus='$ClainStatus',ModifiedDate='$ModifiedDate',ModifiedBy='$user_id' WHERE id = '$id'";
  $conn->query($query2);

   $sql = "UPDATE tbl_insurance_clain SET ClainStatus='$ClainStatus' WHERE id='$CompId'";
  $conn->query($sql);

  echo "<script>alert('Insurance Claim Record Updated Successfully!');window.location.href='view-insurance-claim-action.php?id=$CompId';</script>";
                ...
                <div class="layout-content">
                    ...
                    <div class="container-fluid flex-grow-1 container-p-y">
                        <h4 class="font-weight-bold py-3 mb-0"><?php if($_GET['id']) {?>Edit <?php } else{?> Take
                            <?php } ?> Action On Insurance Claim</h4>
...
<option selected="" value="">Select Customer</option>
 <?php 
  ...
  foreach($row12 as $result){
     ?>
  <option <?php if($row77["CustId"] == $result['id']) {?> selected <?php } ?> value="<?php echo $result['id'];?>">
...
<label class="form-label"> Insurance Claim Documents Status<span class="text-danger">*</span></label>
 <select class="form-control" name="DocumentsStatus" id="DocumentsStatus" disabled>

<option selected="" value="">Select</option>

  <option value="Received" <?php if($row77['DocumentsStatus'] == 'Received'){?> selected <?php } ?>>Received</option>
    <option value="Not Received" <?php if($row77['DocumentsStatus'] == 'Not Received'){?> selected <?php } ?>>Not Received</option>
</select>
<div class="clearfix"></div>
</div>
...
<label class="form-label"> Reason for insurance claim<span class="text-danger">*</span></label>
 <select class="form-control" name="ClainReason" id="ClainReason" disabled>
<option selected="" value="">Select</option>
 <?php 
  ...
  foreach($row12 as $result){
     ?>
  <option <?php if($row77['ClainReason'] == $result['Name']){?> selected <?php } ?> value="<?php echo $result['Name'];?>">
