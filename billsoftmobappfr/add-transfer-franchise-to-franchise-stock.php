...
$FranchiseId = $_SESSION['FranchiseId'];
  ...
  $conn->query($sql);
      ...
      foreach ($_SESSION["cart_item"] as $product){
        $ProdId = $product['FromProdId'];
        $sql = "SELECT ProdId FROM tbl_cust_products_2025 WHERE id='$ProdId'";
        $row = getRecord($sql);
        $MainProdId = $row['ProdId'];
        //$FrProdId = $product['ToProdId'];
        $Qty = $product['Qty'];
        ...
        echo $sql33 = "INSERT INTO tbl_transfer_franchise_prod_stock_items_2025 SET FromFrId='$FromFrId',ToFrId='$ToFrId',ProdId='$ProdId',FrProdId='$MainProdId',Qty='$Qty',Unit='$Unit',CreatedDate='$CreatedDate',TransferId='$UseRawId',CreatedBy='$user_id',StockDate='$StockDate'";
        $conn->query($sql33);
                                
        $sql22 = "INSERT INTO tbl_cust_prod_stock_2025 SET UserId='$FromFrId',MainProdId='$MainProdId',ProdId='$ProdId',Qty='$Qty',Unit='$Unit',CreatedDate='$CreatedDate',TransferId='$UseRawId',Status='Dr',Narration='$Narration',StockDate='$StockDate',CreatedBy='$user_id',FrId='$FromFrId'";
        $conn->query($sql22);
      }
   
echo "<script>alert('Request Sent Successfully!');window.location.href='view-transfer-franchise-to-franchise-stock.php';</script>";
    unset($_SESSION["cart_item"]);
...
<input type="hidden" name="ToFrId" id="ToFrId" value="<?php echo $_REQUEST['ToFrId'];?>">
...
<label class="form-label">Franchise Product</label>
 <select class="selectpicker form-control" name="FromProdId" id="FromProdId1" onchange="getAvailProdStock(this.value,document.getElementById('srno1').value)" data-live-search="true">
<option selected="" value="">Select Product</option>
 <?php 
  $sql12 = "SELECT * FROM tbl_cust_products_2025 WHERE Status='1' AND CreatedBy='$FranchiseId' AND ProdType2=1 AND ProdType=0 AND ProdType2 IN (1,3) ORDER BY ProductName";
  $row12 = getList($sql12);
...
function showData(){
     var action = "showData";
      $.ajax({
                url: "ajax_files/ajax_transfer_stock.php",
                method: "POST",
                data: {
                    action: action
                },
...
function deleteCart(code){
 ...
 function addItem(srno){
    var action = "addItem";
    var FromProdId = $('#FromProdId'+srno).val();
    //var ToProdId = $('#ToProdId'+srno).val();
    ...
    var quantity = 1;
        $.ajax({
                ...
                success: function(data) {
                    console.log(data);
                    showData();
                    //$('#ToProdId'+srno).val('').attr("selected",true);
                    $('#FromProdId'+srno).val('').attr("selected",true);
                    $('#AvailStock'+srno).val('');
