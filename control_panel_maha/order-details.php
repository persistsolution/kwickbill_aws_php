...
if(isset($_POST['confirm_submit'])){
  ...
  $ConfirmDate2 = date("d-M-Y", strtotime(str_replace('-', '/',$ConfirmDate)));
  $EmpId = $_POST['EmpId'];
  $VedId = $_POST['VedId'];
  ...
  $sql21 = "SELECT * FROM confirm_orders WHERE Ordid='$oid'";
      ...
      $sql2 = "UPDATE orders SET OrderProcess='5',ConfirmDate='$ConfirmDate',ConfirmTime='$ConfirmTime' WHERE id='$oid'";
  $conn->query($sql2);

  $sqlc11 = "SELECT Tokens,Phone FROM tbl_users WHERE id='$CustId'";
  $data=mysqli_query($con,$sqlc11);
  while($rowc11=mysqli_fetch_array($data))
  {
  $title = "Your Order Dispatch.";
  $body =  "Dispatch: Your Order has been Dispatch.";
  $reg_id = $rowc11[0];
  ...
  {
  $title = $ordno." Order Dispatch.";
  $body =  "Dispatch: ".$ordno." Order has been Dispatch.";
  $reg_id = $rowc11[0];
  $registrationIds = array($reg_id);
  ...
  $row11 = getRecord($sqlc11);
  $Phone = $row11['Phone'];
  $smstxt = "We are excited to inform you that your Maha Chai order is on its way! Here are the dispatch details: Order No: ".$ordno." Dispatch Date: ".$ConfirmDate2." Estimated Delivery Time: withing 7 Working Days For any query please call us : 8007885000 Thank you for choosing Maha Chai!";
  $dltentityid = "1501701120000037351";
  $dlttempid = "1707169875372985850";
 ...
 if(isset($_POST['submit'])){
  ...
  $DelieverDate2 = date("d-M-Y", strtotime(str_replace('-', '/',$DelieverDate)));
  ...
  $row11 = getRecord($sqlc11);
  $Phone = $row11['Phone'];
  $smstxt = "Good news! Your Maha Chai order has been successfully delivered. We hope you enjoy your purchase. Here are the delivery details: Order No: ".$ordno." Delivery Date: ".$DelieverDate2." If you have any questions or feedback about your order, please call 8007885000 Thank you for choosing Maha Chai";
  $dltentityid = "1501701120000037351";
  $dlttempid = "1707169875390238420";
  include '../incsmsapi.php';  
  ?>
  <script type="text/javascript">
 ...
 <?php }   
  ...
  $body =  "Cancelled: Your Maha Buddy order ".$OrderNo." has been cancelled.";
  $reg_id = $rowc11[0];
  $registrationIds = array($reg_id);
  $url = "$SiteUrl/my-orders-details.php?oid=$oid";
  include '../incnotification.php';
...
<div class="col-md-2 mb-3">
<a href="javascript:void(0)" data-toggle="modal" data-target="#modals-default" class="btn btn-info btn-sm">Order Dispatch</a>
