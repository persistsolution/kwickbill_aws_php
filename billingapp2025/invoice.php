...
<?php 
$id = $_GET['id'];
$sql = "SELECT ti.* FROM tbl_invoice ti LEFT JOIN tbl_users tu ON ti.CustId=tu.id WHERE ti.id='$id'";
$row = getRecord($sql);
...
<?php echo $row['Address'];?></div>
        ...
        <?php 
            ...
            foreach($row12 as $result12){
                $sql13 = "SELECT ProductName FROM tbl_raw_products WHERE id='".$result12['ProdId']."'";
