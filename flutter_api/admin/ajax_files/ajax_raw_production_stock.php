 ...
 <div class="form-group col-md-3">
    <label class="form-label">Production Product </label>
        ...
        <option selected value="">...</option>
            ...
            ?>
        <option <?php if ($row7["CustProdId"] == $result['id']) { ?> selected <?php } ?> value="<?php echo $result['id']; ?>"><?php echo $result['ProductName']." (&#8377;".$result['Price'].")"; ?></option>
       <?php } ?>
    </select>
</div>
...
if($_POST['action'] == 'getAvailRawProductionStock'){
	$id = $_POST['id'];
	$GodownId = $_POST['GodownId'];
$sql = "SELECT * FROM tbl_raw_production_products WHERE id='$id'";
$row = getRecord($sql);
$Unit = $row['Unit'];
$Price = $row['Price'];
$CgstPer = $row['CgstPer'];
...
$sql2 = "SELECT sum(creditqty)-sum(debitqty) AS balqty FROM 
(SELECT (case when Status='Dr' then sum(Qty) else '0' end) as debitqty,(case when Status='Cr' then sum(Qty) else '0' end) as creditqty 
FROM `tbl_production_raw_stock` WHERE ProdId='$id' GROUP by Status) as a";
	$row2 = getRecord($sql2);
	if($row2['balqty'] > 0){
