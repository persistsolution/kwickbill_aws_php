...
if ($_POST['action'] == 'getInvoice') {
    ...
    <?php
    $sql1 = "SELECT * FROM tbl_invoice WHERE CustId='$id'";
    $row1 = getList($sql1);
...
if ($_POST['action'] == 'getInvoiceDetails') {
    $id = $_POST['id'];
    $CustId = $_POST['CustId'];
    $sql = "SELECT NetAmount AS Amount FROM tbl_invoice WHERE InvoiceNo='$id'";
    $row = getRecord($sql);
    $sql2 = "SELECT SUM(Amount) As TotPaidAmt FROM tbl_general_ledger WHERE InvoiceNo='$id' AND Type='PW' AND CrDr='dr' AND UserId='$CustId' AND Flag='Invoice' AND AccRoll='Vendor'";
    $row2 = getRecord($sql2);
    $TotAmt = $row['Amount'];
...
if ($_POST['action'] == 'getCustBalAmt') {
    $uid = $_POST['uid'];
    //paid amt
    $sql2 = "SELECT SUM(Amount) As TotPaidAmt FROM tbl_general_ledger WHERE UserId='$uid' AND CrDr='dr' AND AccRoll='Vendor'";
    $row2 = getRecord($sql2);
    $TotPaidAmt = $row2['TotPaidAmt'];

    //debit amt
    $sql2 = "SELECT SUM(Amount) As TotAmt FROM tbl_general_ledger WHERE UserId='$uid' AND CrDr='cr' AND AccRoll='Vendor'";
    $row2 = getRecord($sql2);
    $TotAmt = $row2['TotAmt'];
    $TotBalAmt = $TotAmt - $TotPaidAmt;
    echo json_encode(array('TotAmt' => $TotAmt, 'TotPaidAmt' => $TotPaidAmt, 'TotBalAmt' => $TotBalAmt));
...
if ($_POST['action'] == 'getCollections') {
    ...
    <table id="example" class="table table-striped table-bordered" style="width:100%">
        ...
        <tbody>
            <?php
            $srno = 1;
            $sql = "SELECT * FROM tbl_general_ledger WHERE UserId='$CustId' AND InvoiceNo='$id' AND Type IN ('PW','OB') AND Flag='Invoice' AND AccRoll='Vendor' ORDER BY id";
            $rx = $conn->query($sql);
...
$sql4 = "INSERT INTO tbl_general_ledger SET SrNo='$SrNo',Code='$Code',UserId='$CustId',
                            ...
                            InvoiceNo='$InvoiceNo',Amount='$Amount',CrDr='dr',Roll=3,
                            Type='PW',CreatedDate='$CreatedDate',PaymentDate='$PayDate',
                            PayMode='$PayType',ChequeNo='$ChequeNo',ChqDate='$ChqDate',BankName='$BankName',UpiNo='$UpiNo',Narration='$Narration',
                            SellId='$SellId',Flag='Invoice',AccRoll='Vendor',CreatedBy='$user_id'";
                        $conn->query($sql4);
                        $PostId = mysqli_insert_id($conn);
                        $sql4 = "INSERT INTO tbl_general_ledger SET SrNo='$SrNo',Code='$Code',UserId='0',
                            AccCode='AC1',
                            AccountName='Company Account',
                            InvoiceNo='$InvoiceNo',Amount='$Amount',CrDr='cr',Roll=3,
                            Type='PW',CreatedDate='$CreatedDate',PaymentDate='$PayDate',
                            PayMode='$PayType',ChequeNo='$ChequeNo',ChqDate='$ChqDate',BankName='$BankName',UpiNo='$UpiNo',Narration='$Narration',
                            SellId='$SellId',Flag='Invoice',AccRoll='Company',PostId='$PostId',CreatedBy='$user_id'";
                        $conn->query($sql4);
