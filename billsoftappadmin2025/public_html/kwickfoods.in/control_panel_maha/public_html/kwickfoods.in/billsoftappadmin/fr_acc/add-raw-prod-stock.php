...
<?php 
  if(isset($_POST['submit'])){
    ...
    if($_GET['id'] == ''){
     $qx = "INSERT INTO tbl_fr_raw_stock SET FrId='$BillSoftFrId',ProdId='$ProdId',Qty='$Qty',Unit='$Unit',CreatedBy='$user_id',StockDate='$StockDate',Narration='$Narration',Status='Cr',CreatedDate='$CreatedDate'";
  $conn->query($qx);
  $InvId = mysqli_insert_id($conn);
  
  $url = $_SERVER['REQUEST_URI'];
   $createddate = date('Y-m-d H:i:s');
   $sql = "INSERT INTO tbl_user_logs SET userid='$user_id',frid='$BillSoftFrId',url='$url',action='raw product stock added',invid='$InvId',createddate='$createddate',roll='raw-product-stock'";
  $conn->query($sql);
  ...
  echo "<script>alert('Product Stock Added Successfully!');window.location.href='view-fr-raw-stock.php';</script>";
    ...
    $url = $_SERVER['REQUEST_URI'];
   $createddate = date('Y-m-d H:i:s');
   $sql = "INSERT INTO tbl_user_logs SET userid='$user_id',frid='$BillSoftFrId',url='$url',action='raw product stock updated',invid='$id',createddate='$createddate',roll='raw-product-stock'";
  $conn->query($sql);
...
<?php 
  $sql12 = "SELECT * FROM tbl_cust_products WHERE Status='1' AND CreatedBy='$BillSoftFrId' AND ProdType='1'";
  $row12 = getList($sql12);
