...
$StateId = addslashes(trim($_POST['StateId']));
...
if($City_Id == 'Other'){
   ...
   $sql = "INSERT INTO tbl_fuel_city SET CountryId=1,StateId='$StateId',Name='$LocationCity',Status='1'";
   $conn->query($sql);
...
if ($_GET['id'] == '') { 
     $qx = "INSERT INTO tbl_fuel_station_details SET 
        ...
        userid='$user_id',points='$cnt',Comments='$Comments',
        TouristLocation='$TouristLocation',MarkLatitude='$MarkLatitude',
        MarkLogitude='$MarkLogitude',MarkAddress='$MarkAddress',StateId='$StateId',CityId='$CityId'";
    ...
    $query2 = "UPDATE tbl_fuel_station_details SET 
        ...
        modifiedby='$user_id',points='$cnt',Comments='$Comments',TouristLocation='$TouristLocation',StateId='$StateId',CityId='$CityId'
        WHERE id = '$id'";
    $conn->query($query2);
    ?>
    <script>
...
<div id="map"></div>
    ...
    <?php if ($_GET['id'] == '') { ?>
      <input type="text" name="MarkLatitude" id="MarkLatitude" class="form-control" placeholder="" value="<?php echo $row7['MarkLatitude'];?>" readonly>
 <input type="text" name="MarkLogitude" id="MarkLogitude" class="form-control" placeholder="" value="<?php echo $row7['MarkLogitude'];?>" readonly>
 <input type="text" name="MarkAddress" id="MarkAddress" class="form-control" placeholder="" value="<?php echo $row7['MarkAddress'];?>" readonly>
...
<input type="hidden" name="Logitude" id="Logitude" class="form-control" placeholder="" value="<?php echo $Longitude;?>" readonly>
 <?php } else { ?>
       <input type="text" name="MarkLatitude" class="form-control" placeholder="" value="<?php echo $row7['MarkLatitude'];?>" readonly>
 <input type="text" name="MarkLogitude" class="form-control" placeholder="" value="<?php echo $row7['MarkLogitude'];?>" readonly>
 <input type="text" name="MarkAddress" class="form-control" placeholder="" value="<?php echo $row7['MarkAddress'];?>" readonly>
  <input type="hidden" name="Latitude" id="Latitude" class="form-control" placeholder="" value="<?php echo $row7['MarkLatitude'];?>" readonly>
             <input type="hidden" name="Logitude" id="Logitude" class="form-control" placeholder="" value="<?php echo $row7['MarkLogitude'];?>" readonly>
 ...
 <br>
        ...
        $q = "select * from tbl_fuel_state WHERE CountryId='1' ORDER BY Name ASC";
        $r = $conn->query($q);
...
?>
 ...
 $StateId = $row7['StateId'];
        $q = "select * from tbl_fuel_city WHERE StateId='$StateId' ORDER BY Name ASC";
        $r = $conn->query($q);
        while($rw = $r->fetch_assoc())
    {
?>
            ...
            <div class="form-group col-md-12 location" style="display:none;">
                <label class="form-label">District Name</label>
                <input type="text" name="LocationCity" id="LocationCity" class="form-control" placeholder="" value="<?php echo $row7['LocationCity'];?>">
            </div>
            <div class="form-group col-md-12">
            ...
            <div class="form-group col-md-12">
                ...
                <select name="FuelStationType" id="FuelStationType" class="form-control">
                    <option value="">Select</option>
                    <option value="BPCL" <?php if($row7['FuelStationType'] == 'BPCL'){?> selected <?php } ?>>BPCL</option>
                    <option value="IOCL" <?php if($row7['FuelStationType'] == 'IOCL'){?> selected <?php } ?>>IOCL</option>
                    <option value="HPCL" <?php if($row7['FuelStationType'] == 'HPCL'){?> selected <?php } ?>>HPCL</option>
                    <option value="OTHERS" <?php if($row7['FuelStationType'] == 'OTHERS'){?> selected <?php } ?>>OTHERS</option>
                </select>
...
<?php include 'footer.php';?>
   ...
   $(document).on("change", "#StateId", function(event) {
        ...
        $(document).on("change", "#CityId", function(event) {
            ...
            if(val == 'Other'){
                $('.location').show();
                $('#LocationCity').attr("required",true);
            }
            else{
                $('.location').hide();
                $('#LocationCity').attr("required",false);
            }
        });
   });
