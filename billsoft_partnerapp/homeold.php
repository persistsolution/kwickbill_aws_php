...
<?php 
 ...
 if($Roll != 1){
    ...
    $sql.=" AND id IN ($CocoFranchiseAccess)";
    }
}
 $row = getList($sql);
 foreach($row as $result){
   $sql2 = "SELECT SUM(NetAmount) AS NetAmount FROM tbl_customer_invoice WHERE InvoiceDate='".date('Y-m-d')."' AND FrId='".$result['id']."'";
   $row2 = getRecord($sql2);
   
   $sql22 = "SELECT * FROM tbl_customer_invoice WHERE InvoiceDate='".date('Y-m-d')."' AND FrId='".$result['id']."'";
   $rncnt22 = getRow($sql22);
   $NetAmount2 = $row2['NetAmount'];
   if($NetAmount2 > 0){
       $NetAmount = $row2['NetAmount'];
   }
   else{
       $NetAmount = 0;
   }
   ...
   $TotNetAmount+=$NetAmount;
    if($NetAmount > 0){
        ...
        $sql = "SELECT Max(modified_time) AS LastTime FROM `tbl_customer_invoice` WHERE FrId='".$result['id']."'";
$row = getRecord($sql);
                            ...
                            <li class="list-group-item" style="border-radius: 10px;">
                                ...
                                <div class="row align-items-center">
                                    ...
                                    <div class="col-auto" align="right" >
                                         <p class="small text-secondary" style="font-size:15px;"><strong>&#8377;<?php echo number_format($NetAmount,2);?></strong><br><span style="font-size:12px;">Avg : <?php echo number_format($NetAmount/$rncnt22,2);?></span></p>
                                    </div>
                                </div>
                            </li>
                            </a>
                    ...
                    <div class=" px-0">
                        <ul class="list-group list-group-flush">
                            ...
                            <li class="list-group-item" style="border-radius: 10px;">
                                ...
                                <div class="row align-items-center">
                                    <div class="col align-self-center pr-0" width="80%">
                                        <h6 class=" mb-1" style="color:black;font-size:14px;font-weight:500;">Today Total Income</h6>
