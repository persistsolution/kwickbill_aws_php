...
<head>
<title><?php echo $Proj_Title; ?> | Cancel Reason</title>
<meta charset="utf-8">
...
<h4 class="font-weight-bold py-3 mb-0">List Of Cancel Reason
...
<span class="font-weight-light">Cancel Reason</span>
</h5>
...
<input type="text" name="Name" class="form-control" id="Name" placeholder="Cancel Reason Name" required>
...
function category_lists(){
  var action = 'view';
      $.ajax({
  type: "POST",
  url: "ajax_files/ajax_cancel_reason.php",
   data:{action:action},  
  ...
  function error_toast(){
   ...
   $.growl.success({
      title:    'Success',
      message:  'New Cancel Reason Added Successfully!',
      location: isRtl ? 'tl' : 'tr'
    });
  }
  ...
  $(document).ready(function() {
    ...
    $('#example').DataTable( {
      ...
      $('#add_button').click(function(){  
           $('.modal-title').html("Add <span class='font-weight-light'>Cancel Reason</span>");  
           $('#action').val("Add");  
           $('#id').val('');
   ...
   swal({
            ...
            if (isConfirm) {
              ...
              category_lists();
                ...
                swal("Cancelled", "Cancel Reason is safe :)", "error");
            }
        });
