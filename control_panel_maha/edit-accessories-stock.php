...
$sql8 = "SELECT GROUP_CONCAT(AccId) AS AccId FROM tbl_accessories_stock WHERE InvId='$id' AND CrDr='cr'";
$row8 = getRecord($sql8);
$SaveProductId = $row8['AccId'];

$query22 = "SELECT count(*) as totrec FROM tbl_accessories_stock WHERE InvId = '$id' AND CrDr='cr'";
...
if($_GET["action"]=="deleteacc")
{
  ...
  ?>
    <script type="text/javascript">
      ...
      window.location.href="edit-accessories-stock.php?id=<?php echo $bid;?>";
    </script>
<?php } 
?>
...
$Narration = addslashes(trim($_POST['Narration']));
     ...
     $qx = "UPDATE tbl_accessories_stock_invoice SET BranchId='$BranchId',TotQty='$TotQty',Status=1,ModifiedBy='$user_id',ModifiedDate='$CreatedDate',DmNo='$DmNo',StockDate='$StockDate',Narration='$Narration' WHERE id='$id'";
  $conn->query($qx);
  $InvId = $id;

$sql = "DELETE FROM tbl_accessories_stock WHERE InvId='$InvId'";
$conn->query($sql);
...
if($number > 0)  
   ...
   echo "<script>alert('Accessories Stock Update Successfully!');window.location.href='view-accessories-stocks.php';</script>";
...
<h4 class="font-weight-bold py-3 mb-0">Update Accessories Stock</h4>
...
<?php 
$i=1;
  $sql12 = "SELECT * FROM tbl_accessories_stock WHERE InvId='$id' AND CrDr='cr'";
  $row12 = getList($sql12);
  foreach($row12 as $result){
     ?>
<div class="form-row">
...
<input type="text" name="Qty[]" id="Qty<?php echo $i; ?>" class="form-control txt" placeholder="e.g.,1" value="<?php echo $result["Qty"];?>" autocomplete="off" min="1" oninput="getSubTotal()">
</div>
...
<div class="form-group col-md-6 ">
<label class="form-label">Accessories Name</label>
 <select class="form-control" name="AccId[]" id="AccId<?php echo $row_cnt; ?>" >
<option selected="" value="">Select Accessories</option>
 <?php 
  $sql12 = "SELECT * FROM tbl_accessories WHERE Status='1'";
  $row12 = getList($sql12);
...
<?php } ?>
                                        ...
                                        <div class="form-group col-md-2">
<label class="form-label">Qty </label>
<input type="number" name="Qty[]" id="Qty<?php echo $row_cnt; ?>" class="form-control txt" placeholder="e.g.,1" value="" autocomplete="off" min="1" oninput="getSubTotal()">
</div>
