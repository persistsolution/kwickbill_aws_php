...
<option selected="" value="all">All</option>
 <?php 
  $sql12 = "SELECT * FROM tbl_cust_sub_category WHERE Status='1' AND CatId='".$_REQUEST['CatId']."'";
  $row12 = getList($sql12);
  foreach($row12 as $result){
     ?>
...
<option value="2" <?php if($_REQUEST["ProdType2"]=='2') {?> selected <?php } ?>>Raw Product</option>
</select>
</div>
...
<table id="example" class="table mb-0 dataTable no-footer" role="grid" aria-describedby="report-table_info">
       <thead class="thead-light">
        ...
        <tbody>
            <?php 
            $sql = "SELECT p.*,c.Name As Category,cs.Name As SubCatName FROM tbl_cust_products p 
                    LEFT JOIN tbl_cust_category c ON c.id=p.CatId 
                    LEFT JOIN tbl_cust_sub_category cs ON cs.id=p.SubCatId WHERE p.CreatedBy='$BillSoftFrId' AND ProdType=0 AND p.delete_flag=0";
 ...
 <!--<td class="align-middle">
              ...
              <td class="align-middle"><?php if($row['ProdType2']=='1'){ echo "<span style='color:green;'>MRP Product</span>";} else { echo "<span style='color:red;'>Raw Product</span>";} ?></td>
