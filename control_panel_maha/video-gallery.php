...
<head>
<title><?php echo $Proj_Title; ?> | Video Gallery</title>
<meta charset="utf-8">
...
<h4 class="font-weight-bold py-3 mb-0">List Of Video Gallery
...
<span class="font-weight-light">Video Gallery</span>
</h5>
...
<label class="form-label">Video For<span class="text-danger">*</span></label>
<select class="form-control" name="VideoFor" id="VideoFor" required="">
<option value="1">Receipe</option>
...
<span class="custom-file-label"></span>
 ...
 <span id="show_photo">
  ...
  <div class="form-row">
<div class="form-group col">
<label class="form-label">Youtube Code (e.g. - qnMKd2ShJLI)<span class="text-danger">*</span></label>
<input type="text" name="Name" class="form-control" id="Name" placeholder="Youtube Code" required>
...
function category_lists(){
  var action = 'view';
      $.ajax({
  type: "POST",
  url: "ajax_files/ajax_video_gallery.php",
   data:{action:action},  
  ...
  function error_toast(){
   ...
   $.growl.error({
      title:    'Error',
      message:  'Video Gallery Already Exists',
   ...
   $.growl.success({
      title:    'Success',
      message:  'New Video Gallery Added Successfully!',
      location: isRtl ? 'tl' : 'tr'
    });
  }
function update_toast(){
  ...
  $(document).ready(function() {
    ...
    $('#example').DataTable( {
      ...
      $('#add_button').click(function(){  
           $('.modal-title').html("Add <span class='font-weight-light'>Video Gallery</span>");  
           $('#action').val("Add");  
           $('#id').val('');
  $('#Photo').val('');
        $('#OldPhoto').val(''); 
      ...
      $('#VideoFor').attr("selected","selected").val('');
       $('#submit').text('Submit');
 ...
 $.ajax({  
                ...
                success:function(data){  
                    ...
                    $('#VideoFor').val(data.VideoFor).attr("selected",true);  
                   ...
                   if(data.Photo==''){
                    ...
                    $('#show_photo').html('<div class="ui-feed-icon-container float-left pt-2 mr-3 mb-3"><a href="javascript:void(0)" class="ui-icon ui-feed-icon ion ion-md-close bg-secondary text-white" id="delete_photo"></a><img src="../uploads/'+data.Photo+'" alt="" class="img-fluid ticket-file-img" style="width: 64px;height: 64px;"></div>');
                     ...
                     $('#action').val('Edit'); 
                       ...
                       $('#submit').text("Update");   
                       $('.insert_frm').modal('show');
                         $('.modal-title').html("Update <span class='font-weight-light'>Video Gallery</span>"); 
 ...
 //alert(id);
   swal({
            title: "Are you sure?",
            text: "You will not be able to recover this Video Gallery!",
            type: "warning",
            ...
            if (isConfirm) {
                ...
                success:function(data){
              swal("Deleted!", "Video Gallery has been deleted.", "success");
              ...
              category_lists();
                ...
                swal("Cancelled", "Video Gallery is safe :)", "error");
            }
        });
