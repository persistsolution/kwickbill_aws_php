...
if($_GET['id']==''){
     $qx = "INSERT INTO tbl_products SET CGST='$CGST',SGST='$SGST',IGST='$IGST',Photo2='$Photo2',ProductCode='$ProductCode',BrandId='$BrandId',CatId='$CatId',ProductName = '$ProductName',Status='$Status',ModelNo='$ModelNo',Photo='$Photo',Price = '$Price',Details='$Details',CreatedDate='$CreatedDate',CreatedBy='$user_id',Code='$Code',Unit='$Unit',Roll='$Roll'";
  $conn->query($qx);
  $id = mysqli_insert_id($conn);
  $PrdNo = "1000".$id;
  $sql = "UPDATE tbl_products SET PrdNo='$PrdNo' WHERE id='$id'";
  $conn->query($sql);
  echo "<script>alert('Product Added Successfully!');window.location.href='view-products.php';</script>";
}
else{
 
    $query2 = "UPDATE tbl_products SET CGST='$CGST',SGST='$SGST',IGST='$IGST',Photo2='$Photo2',ProductCode='$ProductCode',BrandId='$BrandId',CatId='$CatId',ProductName = '$ProductName',Status='$Status',ModelNo='$ModelNo',Photo='$Photo',Price = '$Price',Details='$Details',ModifiedDate='$ModifiedDate',ModifiedBy='$user_id',Code='$Code',Unit='$Unit',Roll='$Roll' WHERE id = '$id'";
  $conn->query($query2);
  echo "<script>alert('Product Updated Successfully!');window.location.href='view-products.php';</script>";
...
<input type="text" name="ModelNo" class="form-control" id="ModelNo" placeholder="Model No" value="<?php echo $row7["ModelNo"]; ?>" required>
<div class="clearfix"></div>
</div>

<div class="form-group col-lg-3">
<label class="form-label">Product Code <span class="text-danger">*</span></label>
<input type="text" name="ProductCode" class="form-control" id="ProductCode" placeholder="" value="<?php echo $row7["ProductCode"]; ?>" required>
<div class="clearfix"></div>
</div>
...
<input type="text" name="Unit" class="form-control" id="Unit" placeholder="Product Unit" value="<?php echo $row7["Unit"]; ?>">
<div class="clearfix"></div>
</div>
...
<input type="text" name="CGST" class="form-control" id="CGST" placeholder="" value="<?php echo $row7["CGST"]; ?>" required>
<div class="clearfix"></div>
</div>
...
<div class="form-group col-md-12">
  <label class="form-label">Broucher </label>
<label class="custom-file">
<input type="file" class="custom-file-input" name="Photo2" style="opacity: 1;">
<input type="hidden" name="OldPhoto2" value="<?php echo $row7['Photo2'];?>" id="OldPhoto2">
<span class="custom-file-label"></span>
</label>
<?php if($row7['Photo2']=='') {} else{?>
  <span id="show_photo2">
<div class="ui-feed-icon-container float-left pt-2 mr-3 mb-3"><a href="javascript:void(0)" class="ui-icon ui-feed-icon ion ion-md-close bg-secondary text-white" id="delete_photo2"></a><a href="../uploads/<?php echo $row7['Photo2'];?>" alt="" target="_new"><?php echo $row7['Photo2'];?></a></div>
</span>
<?php } ?>
</div>
...
<option value="0" <?php if($row7["Roll"]=='0') {?> selected <?php } ?>>Regular Product</option>
<option value="1" <?php if($row7["Roll"]=='1') {?> selected <?php } ?>>Serial No Product</option>
</select>
<div class="clearfix"></div>
</div> 
