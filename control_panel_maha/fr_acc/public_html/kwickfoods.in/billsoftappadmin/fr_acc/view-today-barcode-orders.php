...
<a href="javascript:void(0)" onclick="goToOffline()" class="btn btn-secondary btn-round"><i class="ion ion-md-add mr-2"></i> Add More</a></span><?php } ?>
</h4>
<br>
...
<?php 
$sql = "SELECT SUM(NetAmount) AS NetAmount FROM tbl_customer_invoice WHERE Roll=2 AND Status=1 AND InvoiceDate='".date('Y-m-d')."' AND FrId='$BillSoftFrId' AND PayType='Online Payment'";
$row = getRecord($sql);
?>
...
//echo date('d-m-Y H:i:s');
            ...
            $i=1;
             ...
             $sql = "SELECT * FROM tbl_customer_invoice WHERE Roll=2 AND Status=1 AND InvoiceDate='".date('Y-m-d')."' AND FrId='$BillSoftFrId'  AND NetAmount>0 AND delete_flag=0 AND PayType='Online Payment'";
            /* if($Roll == 1){
                $sql.=" AND FrId=0";
            }
            else{
