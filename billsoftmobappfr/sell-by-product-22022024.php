...
$Calendar = $_REQUEST['calendar'];
 $sql = "SELECT * FROM tbl_cust_products WHERE CreatedBy='$FranchiseId' ORDER BY srno asc";
 $row = getList($sql);
 foreach($row as $result){
     $sql2 = "SELECT SUM(tcid.Total) AS Total,COUNT(tcid.id) AS TotProd FROM tbl_customer_invoice_details tcid INNER JOIN tbl_customer_invoice tci ON tci.id=tcid.InvId WHERE tcid.ProdId='".$result['id']."' AND tci.Roll=2 AND tci.FrId='$FranchiseId'";
