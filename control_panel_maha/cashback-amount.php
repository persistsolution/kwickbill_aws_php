...
$MainPage="Masters";
$Page = "Cashback-Amount";
?>
<!DOCTYPE html>
<html lang="en" class="default-style">
<head>
<title><?php echo $Proj_Title; ?> | Cashback Amount</title>
<meta charset="utf-8">
...
<h4 class="font-weight-bold py-3 mb-0">List Of Cashback Amount
...
<span class="font-weight-light">Cashback Amount</span>
</h5>
...
<label class="form-label">Cashback For <span class="text-danger">*</span></label>
...
<div class="form-row">
<div class="form-group col">
<label class="form-label">Cashback Amount <span class="text-danger">*</span></label>
...
function product_lists(){
  var action = 'view';
      $.ajax({
  type: "POST",
  url: "ajax_files/ajax_cashback_amount.php",
   data:{action:action},  
  ...
  function error_toast(){
   ...
   $.growl.error({
      title:    'Error',
      message:  'Cashback Amount Already Exists',
   ...
   $.growl.success({
      title:    'Success',
      message:  'New Cashback Amount Added Successfully!',
      location: isRtl ? 'tl' : 'tr'
    });
  }
function update_toast(){
   ...
   $.growl.success({
      title:    'Success',
      message:  'Cashback Amount Updated Successfully!',
      location: isRtl ? 'tl' : 'tr'
    });
  }
  ...
  $(document).ready(function() {
    ...
    $('#example').DataTable( {
      ...
      $('#add_button').click(function(){  
           $('.modal-title').html("Add <span class='font-weight-light'>Cashback Amount</span>");  
           $('#action').val("Add");  
           $('#id').val('');
 ...
 $.ajax({  
                ...
                success:function(data){  
                    ...
                    $('#show_photo').html('<div class="ui-feed-icon-container float-left pt-2 mr-3 mb-3"><a href="javascript:void(0)" class="ui-icon ui-feed-icon ion ion-md-close bg-secondary text-white" id="delete_photo"></a><img src="../uploads/'+data.Photo+'" alt="" class="img-fluid ticket-file-img" style="width: 64px;height: 64px;"></div>');
                       ...
                       $('#submit').text("Update");   
                       $('.insert_frm').modal('show');
                         $('.modal-title').html("Update <span class='font-weight-light'>Product</span>"); 
 ...
 //alert(id);
   swal({
            title: "Are you sure?",
            text: "Deleted All Records Related this Product & You will not be able to recover this Product!",
            type: "warning",
