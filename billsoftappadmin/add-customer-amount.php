...
?>
                ...
                <div class="layout-content">
                    ...
                    <div class="container-fluid flex-grow-1 container-p-y">
                        <h4 class="font-weight-bold py-3 mb-0"><?php if($_GET['id']) {?>Edit <?php } else{?> Receiev
                            <?php } ?> Customer Amount</h4>
...
<option selected="" value="">Select Customer</option>
 <?php 
  $sql12 = "SELECT * FROM tbl_users WHERE Status='1' AND Roll=55";
  $row12 = getList($sql12);
...
<option selected="" value="">Select Invoice No</option>
 <?php 
  $sql12 = "SELECT * FROM tbl_invoice WHERE CustId='".$row7['UserId']."'";
  $row12 = getList($sql12);
  foreach($row12 as $result){
     ?>
  <option <?php if($row7['InvoiceNo'] == $result['InvoiceNo']) {?> selected <?php } ?> value="<?php echo $result['InvoiceNo'];?>">
    <?php echo $result['InvoiceNo']; ?></option>
<?php } ?>
</select>
<div class="clearfix"></div>
</div>
   ...
   <label class="form-label">Narration</label>
    ...
    function getCollections(recid){
         ...
         var CustId = $('#CustId').val();
            $.ajax({
                url: "ajax_files/ajax_cust_general_ledger.php",
                method: "POST",
                data: {
                    action: action,
                    id: recid,
...
//console.log(data);exit();
                        if (data == 0) {
                            ...
                            success_toast();
                            setTimeout(function() {
                                window.location.href = 'franchise-amount.php';
                            }, 2000);
                        }
