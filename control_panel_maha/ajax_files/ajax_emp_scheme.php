...
$query = "SELECT * FROM tbl_emp_scheme WHERE Name = '$Name'";
...
$qx = "INSERT INTO tbl_emp_scheme SET Name = '$Name',Status='$Status',Photo='$Photo'";
	$conn->query($qx);
	echo 1;
}
}
...
if($_POST['action'] == 'fetch_record'){
 $id = $_POST['id'];
    $query = "SELECT * FROM tbl_emp_scheme WHERE id = '$id'";
    $result = $conn->query($query);
...
$query = "SELECT * FROM tbl_emp_scheme WHERE Name = '$Name' AND id != '$id'";
$result = $conn->query($query);
$row_cnt = mysqli_num_rows($result);
if($row_cnt > 0){
  echo 0;
  ...
  $query2 = "UPDATE tbl_emp_scheme SET Name = '$Name', Status='$Status',Photo='$Photo' WHERE id = '$id'";
 	$conn->query($query2);
 }
  echo 1;
}
  ...
  if($_POST['action'] == 'delete') {
      ...
      $id = $_POST['id'];
      $query = "DELETE FROM tbl_emp_scheme WHERE id = '$id'";
      $conn->query($query);
...
if($_POST['action'] == 'deletePhoto'){
    $id = $_POST['id'];
    $Photo = $_POST['Photo'];
        $q = "UPDATE tbl_emp_scheme SET Photo='' WHERE id=$id";
        $conn->query($q);
        $src = "../../uploads/$Photo";
        unlink($src);
...
<table id="example" class="table table-striped table-bordered dt-responsive nowrap" style="width:100%">
        ...
        <tbody>
          <?php 
 $srno = 1;
  $sql = "SELECT * FROM tbl_emp_scheme ORDER BY id DESC";
   $rx = $conn->query($sql);
 ...
 <?php }
   ...
   if($_POST['action'] == 'getProdDetails') {
    $id = $_POST['id'];
    $VedId = $_POST['VedId'];
    $sql = "SELECT * FROM tbl_emp_scheme WHERE id='$id'";
    $row = getRecord($sql);
    $Price = $row['Price'];
