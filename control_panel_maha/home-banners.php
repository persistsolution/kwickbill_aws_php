...
<head>
<title><?php echo $Proj_Title; ?> | Home Banner</title>
<meta charset="utf-8">
...
<h4 class="font-weight-bold py-3 mb-0">List Of Home Banner
...
<span class="font-weight-light">Home Banner</span>
</h5>
...
<span class="custom-file-label"></span>
 ...
 <span id="show_photo">
  ...
  <div class="form-row">
<div class="form-group col">
<label class="form-label">Home Banner Url <span class="text-danger">*</span></label>
<input type="text" name="Name" class="form-control" id="Name" placeholder="Home Banner Url">
...
function category_lists(){
  var action = 'view';
      $.ajax({
  type: "POST",
  url: "ajax_files/ajax_home_banners.php",
   data:{action:action},  
  ...
  function error_toast(){
   ...
   $.growl.error({
      title:    'Error',
      message:  'Home Banner Name Already Exists',
    ...
    function success_toast(){
      /*swal({
            title: "Success!",
            text: "New Home Banner has been Added.",
            type: "success",
   ...
   $.growl.success({
      title:    'Success',
      message:  'New Home Banner Added Successfully!',
      location: isRtl ? 'tl' : 'tr'
    });
  }
function update_toast(){
  ...
  $(document).ready(function() {
    ...
    $('#example').DataTable( {
      ...
      $('#add_button').click(function(){  
           $('.modal-title').html("Add <span class='font-weight-light'>Home Banner</span>");  
           $('#action').val("Add");  
           $('#id').val('');
                ...
                success:function(data){  
                   ...
                   $('#Roll').val(data.Roll).attr("selected",true);  
                    ...
                    $('#show_photo').html('<div class="ui-feed-icon-container float-left pt-2 mr-3 mb-3"><a href="javascript:void(0)" class="ui-icon ui-feed-icon ion ion-md-close bg-secondary text-white" id="delete_photo"></a><img src="../uploads/'+data.Photo+'" alt="" class="img-fluid ticket-file-img" style="width: 64px;height: 64px;"></div>');
                       ...
                       $('#submit').text("Update");   
                       $('.insert_frm').modal('show');
                         $('.modal-title').html("Update <span class='font-weight-light'>Home Banner</span>"); 
 ...
 //alert(id);
   swal({
            title: "Are you sure?",
            text: "Deleted All Records Related this Home Banner & You will not be able to recover this Home Banner!",
            type: "warning",
            ...
            if (isConfirm) {
                ...
                success:function(data){
              swal("Deleted!", "Home Banner has been deleted.", "success");
              ...
              category_lists();
                ...
                swal("Cancelled", "Home Banner is safe :)", "error");
            }
        });
...
event.preventDefault();  
if(confirm("Are you sure you want to delete Home Banner Photo?"))  
           {  
