...
$Calendar = $_REQUEST['calendar'];
 $sql = "SELECT SUM(NetAmount) AS NetAmount,SUM(Discount) AS TotDiscount,COUNT(*) AS TotInv,tc.FrId,tu.ShopName,tu.Address FROM tbl_customer_invoice tc INNER JOIN tbl_users_bill tu ON tu.id=tc.FrId WHERE tu.Roll='5' AND tu.OwnFranchise='".$ownfr."' AND tu.id IN($frids)";
if($Roll != 1){
...
$sql.= " UNION ALL SELECT SUM(NetAmount) AS NetAmount,SUM(Discount) AS TotDiscount,COUNT(*) AS TotInv,tc.FrId,tu.ShopName,tu.Address FROM tbl_customer_invoice_2025 tc INNER JOIN tbl_users_bill tu ON tu.id=tc.FrId WHERE tu.Roll='5' AND tu.OwnFranchise='".$ownfr."' AND tu.id IN($frids)";
if($Roll != 1){
    if($CocoFranchiseAccess != '' || $CocoFranchiseAccess != 0){
    $sql.=" AND tu.id IN ($CocoFranchiseAccess)";
    }
