...
if($_POST['action'] == 'getAvailProdStock2'){
	$id = $_POST['id'];
	$FrId = $_POST['FromFrId'];
$sql = "SELECT Unit FROM tbl_cust_products_2025 WHERE id='$id' AND ProdType=0";
$row = getRecord($sql);
$Unit = $row['Unit'];
...
<?php 
    $CountryId = $_POST['id'];
        ...
        $q = "select id,ProductName,'Selling' AS Roll from tbl_cust_products_2025 WHERE CreatedBy = '$CountryId' AND Transfer=1 AND ProdType=0 AND ProdType2 IN (1,3) ORDER BY ProductName";
        $r = $conn->query($q);
        while($rw = $r->fetch_assoc())
    {
?>
                <option value="<?php echo $rw['id']; ?>"><?php echo $rw['ProductName']." (".$rw['Roll'].")"; ?></option>
<?php } } 
...
?>
    <option value="" selected="selected" disabled="">Select Product</option>
<?php 
         $q = "select * from tbl_cust_products_2025 WHERE CreatedBy = '$ToFrId' AND Transfer=1 AND ProdType=0 AND ProdType2 IN (1,3) AND checkstatus=1 ORDER BY ProductName";
        $r = $conn->query($q);
        while($rw = $r->fetch_assoc())
    {
?>
                <option value="<?php echo $rw['id']; ?>" <?php if($row['id'] == $rw['id']){?>selected<?php } ?>><?php echo $rw['ProductName']; ?></option>
<?php } } 
?>