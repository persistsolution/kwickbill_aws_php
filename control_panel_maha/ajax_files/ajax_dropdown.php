...
<?php 
    $CatId = $_POST['catid'];
        $q = "select * from products WHERE CatId ='$CatId' AND Status='1'";
        $r = $conn->query($q);
        while($rw = $r->fetch_assoc())
    {
?>
...
<?php 
    $CountryId = $_POST['id'];
        $q = "select * from tbl_sub_zone WHERE CatId = '$CountryId' AND Status='1'";
        $r = $conn->query($q);
        while($rw = $r->fetch_assoc())
    {
?>
...
if($_POST['action'] == 'getBankDetails'){
    $id = $_POST['id'];
    $sql = "SELECT * FROM tbl_bank_details WHERE id=$id";
    $row = getRecord($sql);
    echo json_encode($row);
}
