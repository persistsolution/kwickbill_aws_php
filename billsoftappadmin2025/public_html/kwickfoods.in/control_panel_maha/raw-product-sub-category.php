...
<?php 
        $q = "select * from tbl_raw_prod_category WHERE Status='1' AND CreatedBy='$BillSoftFrId'";
        $r = $conn->query($q);
...
function category_lists(){
  var action = 'view';
      $.ajax({
  type: "POST",
  url: "ajax_files/ajax_raw_prod_sub_category.php",
   data:{action:action},  
 ...
 $.ajax({  
                ...
                success:function(data){  
                   ...
                   var res = JSON.parse(data);
                     $('#Name').val(res.Name);  
                      $('#OldPhoto').val(res.Photo); 
                      $('#Photo').val(''); 
                    $('#Status').val(res.Status).attr("selected",true);  
                     $('#CatId').val(res.CatId).attr("selected",true);  
                 
                     $('#action').val('Edit'); 
                    if(res.Photo==''){   $('#show_photo').hide();} else{
                         $('#show_photo').show();
                    $('#show_photo').html('<div class="ui-feed-icon-container float-left pt-2 mr-3 mb-3"><a href="javascript:void(0)" class="ui-icon ui-feed-icon ion ion-md-close bg-secondary text-white" id="delete_photo"></a><img src="../uploads/'+res.Photo+'" alt="" class="img-fluid ticket-file-img" style="width: 64px;height: 64px;"></div>');
                  }
                       $('#id').val(id);  
                       $('#submit').text("Update");   
                       $('.insert_frm').modal('show');
