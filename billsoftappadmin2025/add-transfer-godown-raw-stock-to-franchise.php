...
if(isset($_POST['submit'])){
    ...
    $sql = "INSERT INTO tbl_transfer_godown_raw_prod_stock_2025 SET GodownId='$GodownId',FranchiseId='$FranchiseId',StockDate='$StockDate',TotQty='$TotQty',Narration='$Narration',CreatedBy='$user_id',CreatedDate='$CreatedDate',TotalAmount='$TotalAmount',InvoiceNo='$InvoiceNo',GstAmount='$GstAmount',OwnShop=1";
    $conn->query($sql);
 ...
 foreach ($_SESSION["cart_item"] as $product) {
     ...
     $row = getRecord($sql);
                    ...
                    echo "<script>alert('Record Saved Successfully!');window.location.href='view-transfer-godwon-raw-stock-coco.php';</script>";
}
...
<h4 class="font-weight-bold py-3 mb-0">Transfer MRP Stock Godown To Franchise</h4>
...
<?php 
  $sql12 = "SELECT id,Fname FROM tbl_users_bill WHERE Roll=93";
  $row12 = getList($sql12);
...
<label class="form-label"> Franchise <span class="text-danger">*</span></label>
 <select class="select2-demo form-control" name="FranchiseId" id="FranchiseId" required onchange="getFrTransferProduct(this.value)">

<option selected="" value="">Select Franchise</option>
<?php 
  $sql12 = "SELECT id,ShopName,OwnFranchise FROM tbl_users WHERE Roll=5 AND Status=1";
  $row12 = getList($sql12);
  foreach($row12 as $result){
     ?>
...
<option selected="" value="0">Select Product</option>
 <?php 
  $sql12 = "SELECT id,ProductName FROM tbl_cust_products2 WHERE Status=1 AND ProdType=0 AND ProdType2 IN (3) ORDER BY ProductName";
  $row12 = getList($sql12);
  foreach($row12 as $result){
     ?>
...
<label class="form-label">Franchise Product</label>
 <select class="select2-demo form-control ProdId" name="FrProdId[]" id="FrProdId1" >
<option selected="" value="0">Select Product</option>
  <?php 
  $sql12 = "SELECT id,ProductName FROM tbl_cust_products2 WHERE Status=1 AND ProdType=1 ORDER BY ProductName";
  $row12 = getList($sql12);
  foreach($row12 as $result){
     ?>
  <option value="<?php echo $result['id'];?>">
...
<?php include_once 'footer_script.php'; ?>
  ...
  function addToCart(){
    ...
    var SgstAmt = Number(GstAmt) / 2;
     ...
     if(ProdId==0){
         ...
         alert("Please Select Franchise Product");
     }
     else if(quantity==''){
     ...
     $('#add').text('Please Wait...');
                ...
                success: function(data) {
                 ...
                 $('#AvailStockUnit1').val('');
                 $('#QtyUnit1').val('');
                 $('#FrProdId1').val(0).attr("selected",true);
      ...
      var action = "calTotalQty";
            $.ajax({
                ...
                success: function(data) {
                    ...
                    $('#TotQty').val(res.Qty);
                    $('#TotalAmount').val(parseFloat(res.TotalPrice).toFixed(2));
                    $('#GstAmount').val(parseFloat(res.GstAmt).toFixed(2));
                },
 ...
 function getTotAmt(){
    ...
    function matchMrpProduct(godown_prodid){
        var action = "matchMrpProduct";
        var FrId = $('#FranchiseId').val();
        $.ajax({
                url: "ajax_files/ajax_godown_stock.php",
                method: "POST",
                data: {
                    action: action,
                    godown_prodid: godown_prodid,
                    FrId:FrId
                },
                success: function(data) {
                  //alert(data);
    ...
    function getFrTransferProduct(val){
        var action = "getFrTransferProduct";
        $.ajax({
                url: "ajax_files/ajax_godown_stock.php",
                method: "POST",
                data: {
