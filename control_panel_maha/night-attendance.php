...
$MainPage="Night-Attendance";
$Page = "Night-Attendance";
?>
<!DOCTYPE html>
...
$CreditSalaryStatus = $row['CreditSalaryStatus'];
  ...
  $ToDate3 = strtotime($ToDate2."".$ToTime);
  $FromDate3 = strtotime($FromDate."".$FromTime);
if($FromDate == $ToDate2){
    $ToDate =  $_POST['ToDate'];
}
else{
if($ToDate3>$FromDate3){ 
$ToDate = date('Y-m-d', strtotime('-1 day', strtotime($ToDate2)));
}
else{
   $ToDate =  $_POST['ToDate'];
}
    ...
    $rncnt = getRow($sql);
    if($rncnt > 0){
        $sql2 = "UPDATE tbl_attendance SET AttRoll=1,Salary='$PerDaySalary',Status='$Status',CreatedTime='$FromTime',Type=1 WHERE UserId='$UserId' AND CreatedDate='$FromDate' AND Type=1";
        $conn->query($sql2);
         
       
    }
    else{
       $sql2 = "INSERT INTO tbl_attendance SET AttRoll=1,Salary='$PerDaySalary',Status='$Status',UserId='$UserId',CreatedDate='$FromDate',Latitude='$Latitude',Longitude='$Longitude',Address='',CreatedTime='$FromTime',Latemark='0',HalfDay='0',Type=1,Photo='',TempPrdId=''";
        $conn->query($sql2);
    ...
    $rncnt = getRow($sql);
    if($rncnt > 0){
        $sql2 = "UPDATE tbl_attendance SET AttRoll=1,Salary='$PerDaySalary',Status='$Status',CreatedTime='$ToTime',Type=2 WHERE UserId='$UserId' AND CreatedDate='$ToDate' AND Type=2";
        $conn->query($sql2);
         
       
    }
    else{
       $sql2 = "INSERT INTO tbl_attendance SET AttRoll=1,Salary='$PerDaySalary',Status='$Status',UserId='$UserId',CreatedDate='$ToDate',Latitude='$Latitude',Longitude='$Longitude',Address='',CreatedTime='$ToTime',Latemark='0',HalfDay='0',Type=2,Photo='',TempPrdId=''";
        $conn->query($sql2);
      
    }
     echo "<script>alert('Attendance Saved Successfully');window.location.href='night-attendance.php';</script>";        
...
?>
                ...
                <div class="layout-content">
                    ...
                    <div class="container-fluid flex-grow-1 container-p-y">
                        <h4 class="font-weight-bold py-3 mb-0">Night Attendance</h4>
