...
$CurrDate = date('Y-m-d');
$CurrTime = date('H:i:s');
//$PrevDate = date('Y-m-d', strtotime('-1 day', strtotime($CurrDate)));
if($CurrTime>='12:00:00' && $CurrTime<='23:59:59'){
    $startdate = date('Y-m-d');
}
else{
    $startdate = date('Y-m-d', strtotime('-1 day', strtotime($CurrDate)));
}

$sql8 = "SELECT ta.*,tu.Fname FROM tbl_attendance ta INNER JOIN tbl_users tu ON ta.UserId=tu.id WHERE ta.UserId='$UserId' AND ta.CreatedDate='$startdate' AND Type=1";
$rncnt8 = getRow($sql8);
$row8 = getRecord($sql8);

/*if($CurrTime<='23:59:59'){
    $startdate = date('Y-m-d');
}
else{
    $startdate = date('Y-m-d', strtotime('-1 day', strtotime($CurrDate)));
...
$sql81 = "SELECT ta.*,tu.Fname FROM tbl_attendance ta INNER JOIN tbl_users tu ON ta.UserId=tu.id WHERE ta.UserId='$UserId' AND ta.CreatedDate='$startdate' AND Type=1";
$rncnt81 = getRow($sql81);
if($rncnt81 > 0){
    $sql9 = "SELECT ta.*,tu.Fname FROM tbl_attendance ta INNER JOIN tbl_users tu ON ta.UserId=tu.id WHERE ta.UserId='$UserId' AND ta.CreatedDate='$PrevDate' AND Type=2";
$rncnt9 = getRow($sql9);
$row9 = getRecord($sql9);
}*/
if($CurrTime>='12:00:00' && $CurrTime<='23:59:59'){ 
...
$rncnt9 = getRow($sql9);
$row9 = getRecord($sql9);
$sql7 = "SELECT ta.*,tu.Fname FROM tbl_attendance ta INNER JOIN tbl_users tu ON ta.UserId=tu.id WHERE ta.UserId='$UserId' AND ta.CreatedDate='$startdate'  GROUP BY ta.CreatedDate ORDER BY ta.CreatedDate DESC LIMIT 1";
$rncnt7 = getRow($sql7);
$row7 = getRecord($sql7);
?>
        <div class="main-container">
            <div class="container">
             ...
             <?php include 'eve-start-attendance.php';include 'eve-end-attendance.php';?>
...
<?php 
/*if($rncnt8 > 0) { 
    if($CurrTime>'21:33:00' && '17:00:00'<=$CurrTime){ 
        echo "no"; 
    } else {
        echo "yes"; 
    }
    ...
    ?>
                 ...
                 <button type="button" class="btn btn-sm btn-default rounded" data-toggle="modal" data-target="#myModal" 
                 <?php if($rncnt8 > 0){?> disabled <?php } else if($CurrTime>'12:00:00'){} else {?>disabled <?php } ?>>Start Attendance</button>
                 </div>
                 <div class="col-lg-6 col-6">
                 <button type="button" class="btn btn-sm btn-default rounded" data-toggle="modal" data-target="#myModal2" 
                 <?php if($rncnt9 > 0){?> disabled 
                 <?php } else if($rncnt8 > 0) { } else {?> 
                 disabled <?php } ?>>End Attendance</button>
                 </div>
                 </div>
...
function uploadImageSingle(prdid){
 ...
 $(document).ready(function() {
     $('#validation-form').on('submit', function(e){
      e.preventDefault();    
     $.ajax({  
                url :"ajax_files/ajax_eve_attendance.php",  
                method:"POST",  
                  ...
                  //console.log(data);exit();
                    ...
                    toastr.success('Attendance Successfully!', 'Success', {timeOut: 5000}); 
                        setTimeout(function(){ 
                                        ...
                                        window.location.href="evening-attendance.php";
                                        }, 3000);
                     }
                     else{
