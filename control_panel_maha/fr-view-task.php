...
if($_REQUEST["action"]=="delete")
{
  ...
  ?>
    <script type="text/javascript">
      ...
      window.location.href="fr-view-task.php";
    </script>
<?php } ?>
...
<a href="fr-add-task.php" class="btn btn-secondary btn-round"><i class="ion ion-md-add mr-2"></i> Add New</a></span><?php } ?>
</h4>
...
<table id="example" class="table table-striped table-bordered dt-responsive nowrap" style="width:100%">
        ...
        <tbody>
            <?php 
            $i=1;
            $sql = "SELECT tp.*,tc.Fname FROM tbl_task_new tp 
            LEFT JOIN tbl_users tc ON tc.id=tp.UserId WHERE tc.UnderFrId IN ($CocoFranchiseAccess)";
            if($_POST['ExeId']){
                $ExeId = $_POST['ExeId'];
                if($ExeId == 'all'){
                    $sql.= " ";
           ...
           if($_POST['FromDate']){
            ...
            <tr>
              ...
              <td><?php echo date("d/m/Y", strtotime(str_replace('-', '/',$row['TaskDate']))); ?></td>
                 <td><?php if($row['Status']=='1'){echo "<span style='color:green;'>Completed</span>";} else { echo "<span style='color:red;'>Pending</span>";} ?></td>
            ...
            <td></td>
           <?php } ?>
               <?php if($row['ActionDate']!=''){?>
           <td><?php echo date("d/m/Y", strtotime(str_replace('-', '/',$row['ActionDate']))); ?></td>
           <?php } else{?>
            <td></td>
           <?php } ?>
