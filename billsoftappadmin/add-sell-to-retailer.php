...
<?php 
$sql = "SELECT MAX(id) AS MaxId FROM tbl_sell_retailer";
$row = getRecord($sql);
...
if(isset($_POST['submit'])){
    ...
    $sql = "INSERT INTO tbl_sell_retailer SET GodownId='$GodownId',FranchiseId='$FranchiseId',StockDate='$StockDate',TotQty='$TotQty',Narration='$Narration',CreatedBy='$user_id',CreatedDate='$CreatedDate',TotalAmount='$TotalAmount',InvoiceNo='$InvoiceNo',GstAmount='$GstAmount'";
    $conn->query($sql);
    $UseRawId = mysqli_insert_id($conn);

    $number = count($_POST["GodownProdId"]);
                    if ($number > 0) {
                        for ($i = 0; $i < $number; $i++) {
                            if (trim($_POST["GodownProdId"][$i] != '')) {
                                ...
                                $sql33 = "INSERT INTO tbl_sell_retailer_items SET GodownId='$GodownId',FranchiseId='$FranchiseId',ProdId='$ProdId',FrProdId='$FrProdId',Qty='$Qty',Unit='$Unit',CreatedDate='$CreatedDate',TransferId='$UseRawId',CreatedBy='$user_id',StockDate='$StockDate',Price='$Price',TotalPrice='$TotalPrice',CgstPer='$CgstPer',SgstPer='$SgstPer',IgstPer='$IgstPer',CgstAmt='$CgstAmt',SgstAmt='$SgstAmt',IgstAmt='$IgstAmt',GstAmt='$GstAmt'";
                                $conn->query($sql33);
                                
                                $sql22 = "INSERT INTO tbl_godown_raw_prod_stock SET GodownId='$GodownId',FranchiseId='$FranchiseId',ProdId='$ProdId',Qty='$Qty',Unit='$Unit',CreatedDate='$CreatedDate',RetailerId='$UseRawId',Status='Dr',Narration='$Narration',StockDate='$StockDate',CreatedBy='$user_id',Price='$Price',TotalPrice='$TotalPrice',CgstPer='$CgstPer',SgstPer='$SgstPer',IgstPer='$IgstPer',CgstAmt='$CgstAmt',SgstAmt='$SgstAmt',IgstAmt='$IgstAmt',GstAmt='$GstAmt'";
                                $conn->query($sql22);
                            }
                        }
                    }

                    echo "<script>alert('Record Saved Successfully!');window.location.href='view-sell-to-retailer.php';</script>";
}
...
<h4 class="font-weight-bold py-3 mb-0">Sell To Retailer</h4>
...
<?php 
  $sql12 = "SELECT * FROM tbl_users_bill WHERE Roll=95 AND Status=1";
  $row12 = getList($sql12);
  foreach($row12 as $result){
     ?>
...
<div class="input-group">
    <input type="text" name="Qty[]" id="Qty1" class="form-control txt" placeholder="" value="" autocomplete="off" required oninput="calTotalPrice(document.getElementById('srno1').value,document.getElementById('CgstPer1').value,document.getElementById('SgstPer1').value,document.getElementById('IgstPer1').value)">
        <div class="input-group-append">
...
<label class="form-label">Price </label>
    <input type="text" name="Price[]" id="Price1" class="form-control" placeholder="" value="" autocomplete="off" required oninput="calTotalPrice(document.getElementById('srno1').value,document.getElementById('CgstPer1').value,document.getElementById('SgstPer1').value,document.getElementById('IgstPer1').value)">
        
</div>

<div class="form-group col-md-2">
<label class="form-label">Total Price </label>
    <input type="text" name="TotalPrice[]" id="TotalPrice1" class="form-control txt2" placeholder="" value="" autocomplete="off" required >
        
</div>

<input type="hidden" class="form-control" name="CgstPer[]" id="CgstPer1" value="">
<input type="hidden" class="form-control" name="SgstPer[]" id="SgstPer1" value="">
<input type="hidden" class="form-control" name="IgstPer[]" id="IgstPer1" value="">
