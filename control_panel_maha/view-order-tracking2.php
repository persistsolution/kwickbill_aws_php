...
$Oid = $_GET['oid'];
$pid = $_GET['pid'];
$sql1 = "SELECT OrderNo FROM orders WHERE id='$Oid'";
$row1 = getRecord($sql1);
$ProdName = $row1['OrderNo'];
...
$sql2 = "SELECT Step2,Step3,Step4 FROM orders WHERE id='$oid'";
$row2 = getRecord($sql2);
...
if($OrderStatus == 5){
    if($_POST['DispatchDate']=='0000-00-00'){
        $DispatchDate = date('Y-m-d');$DispatchTime = date('h:i a');  
    }
    else{
        $DispatchDate = $_POST['DispatchDate'];
    }
    if($_POST['DispatchTime']==''){
...
$DelieverTime = date('h:i a');
}
$sql2 = "UPDATE orders SET Step2='$Step2',Step3='$Step3',Step4='$Step4',OrderProcess='$OrderStatus',Step2Date='$Step2Date',Step3Date='$Step3Date',Step4Date='$Step4Date',Step2By='$Step2By',Step3By='$Step3By',Step4By='$Step4By',Photo='$TrackPhoto',DelieverDate='$DelieverDate',DelieverTime='$DelieverTime',DispatchDate='$DispatchDate',DispatchTime='$DispatchTime' WHERE id='$oid'";
$conn->query($sql2);

  
  
echo "<script>alert('Tracking Status Updated Successfully!');window.location.href='view-order-tracking2.php?oid=$oid';</script>";
}
 ?>
<div class="card">
...
<div class="col-lg-6 col-md-6">
    <div class="card ui-timeline mb-4">
      <h5 class="card-header">Order No : <?php echo $ProdName; ?></h5>
      <div class="card-body">
     ...
     <?php 
      ...
      foreach($row as $result){
        if($result['OrderProcess']=='2') {
          $OrderStatus = "Product In Progress!";

        }
        else if($result['OrderProcess']=='5') {
          $OrderStatus = "Product Dispatch!";
        }
        else{
          $OrderStatus = "Product Delivered!";
        }
        if($result['DelieverDate']!='0000-00-00'){
            $UpdatedDate = $result['DelieverDate'];
            $UpdatedTime = $result['DelieverTime'];
        }
        else{
          $UpdatedDate = $result['OrderDate'];  
          $UpdatedTime = $result['OrderTime'];
        }
        if($result['DelieverTime']!=''){
            $DelieverTime = $result['DelieverTime'];
        }
        else{
          $DelieverTime = date('h:i a');  
        }
      ?>     
        ...
        <div class="col-auto text-right update-meta">
          <p class="text-muted mb-0 d-inline"><?php echo date("d/m/Y", strtotime(str_replace('-', '/',$result['OrderDate'])));?></p>
          <?php if($result['OrderProcess'] == 1) {?>
           <i class="fa fa-check bg-success update-icon"></i>
         <?php } else if($result['OrderProcess'] == 5) {?>
        ...
        <?php } else{?>
          ...
          <p class="text-muted mb-0"><?php echo $OrderStatus; ?> Updated at <?php echo $UpdatedTime;?>,<?php echo date("d M", strtotime(str_replace('-', '/',$UpdatedDate)));?></p>
        <?php } ?>
          <div class="bs-vertical-wizard" style="padding-bottom: 10px;">
                       ...
                       <?php include 'inctrackmodel2.php'; ?>
        </div>
      </div>
    <?php } ?>
