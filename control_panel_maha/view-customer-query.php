...
if($_REQUEST["action"]=="delete")
{
  ...
  ?>
    <script type="text/javascript">
      ...
      window.location.href="view-customer-query.php";
    </script>
<?php } ?>
...
<h4 class="font-weight-bold py-3 mb-0">View Customer Query List
    <?php if(in_array("14", $Options)) {?>   
<span style="float: right;">
<a href="add-customer-query.php" class="btn btn-secondary btn-round"><i class="ion ion-md-add mr-2"></i> Add New</a></span><?php } ?>
</h4>
...
<table id="example" class="table table-striped table-bordered dt-responsive nowrap" style="width:100%">
        ...
        <tbody>
            <?php 
            $i=1;
            $sql = "SELECT tp.*,tc.Fname,td.Name FROM tbl_customer_queries tp 
            LEFT JOIN tbl_users tc ON tc.id=tp.CustId 
            LEFT JOIN tbl_diapostion td ON td.id=tp.Diaposition WHERE tp.Status=1";
            if($_POST['ExeId']){
                $ExeId = $_POST['ExeId'];
                if($ExeId == 'all'){
                    $sql.= " ";
           ...
           <?php if(in_array("10", $Options) || in_array("11", $Options)) {?>
            <td>
              ...
              <a href="add-customer-query.php?id=<?php echo $row['id']; ?>" data-toggle="tooltip" data-placement="top" title="" data-original-title="Edit"><i class="lnr lnr-pencil mr-2"></i></a>
...
<script type="text/javascript">
 function getFeedback(id){
    setTimeout(function() {
        window.open(
            'add-customer-query-feedback.php?id=' + id, 'stickerPrint',
