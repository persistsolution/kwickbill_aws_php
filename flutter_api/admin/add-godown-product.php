...
<?php 
$id = $_GET['id'];
$sql7 = "SELECT * FROM tbl_godown_products WHERE id='$id'";
...
if($_GET['id']==''){
    ...
    $qx="INSERT INTO tbl_godown_products SET Qty='$Qty',ProductName = '$ProductName',Status='$Status',CreatedDate='$CreatedDate',CreatedBy='$user_id',Unit='$Unit',ProdType='$ProdType',Price = '$Price',CgstPer='$CgstPer',SgstPer='$SgstPer',IgstPer='$IgstPer',GstAmt='$GstAmt',CgstAmt='$CgstAmt',SgstAmt='$SgstAmt',IgstAmt='$IgstAmt',ProdPrice='$ProdPrice',CatId='$CatId',SubCatId='$SubCatId'";
  $conn->query($qx);
 
      
  echo "<script>alert('Product Added Successfully!');window.location.href='view-godown-products.php';</script>";
}
else{
 
   
    $query2 = "UPDATE tbl_godown_products SET Qty='$Qty',ProductName = '$ProductName',Status='$Status',ModifiedBy='$user_id',ModifiedDate='$CreatedDate',Unit='$Unit',ProdType='$ProdType',Price = '$Price',CgstPer='$CgstPer',SgstPer='$SgstPer',IgstPer='$IgstPer',GstAmt='$GstAmt',CgstAmt='$CgstAmt',SgstAmt='$SgstAmt',IgstAmt='$IgstAmt',ProdPrice='$ProdPrice',CatId='$CatId',SubCatId='$SubCatId' WHERE id = '$id'";
  $conn->query($query2);
  
  
      
  echo "<script>alert('Product Updated Successfully!');window.location.href='view-godown-products.php';</script>";
...
<h4 class="font-weight-bold py-3 mb-0">Godown Product</h4>
...
<?php 
        $q = "select * from tbl_godown_prod_category WHERE Status='1'";
        $r = $conn->query($q);
        while($rw = $r->fetch_assoc())
    {
        
?>
                <option <?php if($rw['id'] == $row7["CatId"]){?> selected <?php } ?> value="<?php echo $rw['id']; ?>"><?php echo $rw['Name']; ?></option>
              <?php } ?></select>
<div class="clearfix"></div>
</div>
...
<option selected="" disabled="" value="">Select Category</option>
<?php 
        $q = "select * from tbl_godown_prod_sub_category WHERE Status='1' AND CatId='".$row7["CatId"]."'";
        $r = $conn->query($q);
        while($rw = $r->fetch_assoc())
    {
        
?>
                <option <?php if($rw['id'] == $row7["SubCatId"]){?> selected <?php } ?> value="<?php echo $rw['id']; ?>"><?php echo $rw['Name']; ?></option>
              <?php } ?></select>
<div class="clearfix"></div>
</div>
...
<datalist id="browsers">
    <?php
                $sql4 = "SELECT DISTINCT(Unit) AS Unit FROM tbl_godown_products WHERE Unit!=''";
                $row4 = getList($sql4);
