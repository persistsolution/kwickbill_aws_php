...
$CreatedDate = date('Y-m-d');
 ...
 echo "<script>alert('Approved Successfully!');window.location.href='coco-manager-less-than-pending-expense-request.php';</script>";
  }
  else if($_GET['page'] == 'cocomore'){
 echo "<script>alert('Approved Successfully!');window.location.href='coco-manager-more-than-pending-expense-request.php';</script>";
  }
  else if($_GET['page'] == 'newexe'){
 echo "<script>alert('Approved Successfully!');window.location.href='new-execution-manager-pending-expense-request.php';</script>";
  }
   else if($_GET['page'] == 'regexe'){
 echo "<script>alert('Approved Successfully!');window.location.href='regular-manager-pending-expense-request.php';</script>";
  }
     ...
     /* if($ManagerStatus == 1){
   if($MgrAmount<=5000){
       $query2 = "UPDATE tbl_expense_request SET AdminApproveDate='$ApproveDate',AdminComment='$MannagerComment',AccAmount='$MgrAmount',AdminStatus='$ManagerStatus',AccBy='$user_id' WHERE id = '$id'";
  $conn->query($query2);
  $sql = "DELETE FROM wallet WHERE UserId='$EmpId' AND ExpId='$id'";
  $conn->query($sql);
  $Narration = "Amount Deduct against Expense For ".$row7["Narration"];
  $sql = "INSERT INTO wallet SET UserId='$EmpId',Amount='$MgrAmount',Narration='$Narration',Status='Dr',CreatedDate='$CreatedDate',CreatedTime='$CreatedTime',ExpId='$id'";
  $conn->query($sql);
   }
  }*/
  
  echo "<script>alert('Approved Successfully!');window.location.href='manager-pending-expense-request.php';</script>";
}
 ...
 <tr>
   ...
   <td><?php if($result["Photo"] == '') {?>
            ...
            <td><?php if($result["Photo4"] == '') {?>
                ...
                <span style="color:red;">No Image Found</span>
             <?php } ?></td>
             <td><a href="<?php echo $result['PdfLink'];?>" target="_blank">Download</a></td>
