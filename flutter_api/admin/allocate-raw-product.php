...
<?php 
    $frid = $_GET['frid'];
    $sql = "DELETE FROM tbl_temp_allocated_fr_raw_prd WHERE FrId='$frid' AND sessionid='$sessionid'";
    $conn->query($sql);
    
    $sql = "SELECT * FROM tbl_cust_products2 WHERE ProdType=1";
    $row = getList($sql);
    foreach($row as $result){
        $sql = "INSERT INTO tbl_temp_allocated_fr_raw_prd SET checkstatus=1,ProdId='".$result['id']."',FrId='$frid',sessionid='$sessionid'";
        $conn->query($sql);
    }
?>
...
<table id="example" class="table table-striped table-bordered dt-responsive nowrap" style="width:100%">
        ...
        <tbody>
            <?php 
            $frid = $_GET['frid'];
             $sql = "SELECT tp.*,tc.Name As CatName,tsc.Name As SubCatName,ta.checkstatus FROM tbl_temp_allocated_fr_raw_prd ta 
INNER JOIN tbl_cust_products2 tp ON tp.id=ta.ProdId 
LEFT JOIN tbl_cust_category_2025 tc ON tc.id=tp.CatId 
LEFT JOIN tbl_cust_sub_category_2025 tsc ON tsc.id=tp.SubCatId WHERE ta.FrId='$frid' AND ta.sessionid='$sessionid' AND tp.Status=1";
            $res = $conn->query($sql);
  ...
  <td><label class="custom-control custom-checkbox">
                                                    <input type="checkbox" id="Check_Id<?php echo $row['id']; ?>" value="1" class="custom-control-input is-valid" onclick="featured(<?php echo $row['id']; ?>)" <?php if($row['checkstatus'] == 1){?> checked <?php } ?>>
                                                    <span class="custom-control-label">&nbsp;</span>
                                                </label></td>
                                                <input type="hidden" value="<?php echo $row['checkstatus']; ?>" name="CheckId[]" id="CheckId<?php echo $row['id']; ?>">
