...
//   $conn->query($sql11);
//   $sql11 = "DELETE FROM tbl_customer_invoice_details_temp WHERE InvId = '$id'";
//   $conn->query($sql11);
...
$modified_time = gmdate('Y-m-d H:i:s.').gettimeofday()['usec'];
    ...
    $conn->query($sql11);
    $sql11 = "UPDATE tbl_customer_invoice_details_temp SET delete_flag=1,modified_time='$modified_time',push_flag=1 WHERE InvId = '$id'";
    $conn->query($sql11);
...
<table id="example" class="table mb-0 dataTable no-footer" role="grid" aria-describedby="report-table_info">
        ...
        <tbody>
            <?php 
                ...
                $sql = "SELECT tcid.*,tcp.ProductName FROM tbl_customer_invoice_details_temp_temp tcid INNER JOIN tbl_cust_products_2025 tcp ON tcid.ProdId=tcp.id WHERE tcid.ServerInvId='".$_GET['id']."'";  
