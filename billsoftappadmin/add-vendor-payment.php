...
$id = $_GET['id'];
$recid = $_GET['recid'];
$sql7 = "SELECT * FROM tbl_vendor_ledger WHERE id='$id'";
$row7 = getRecord($sql7);
if($_GET['id'] == ''){
    $PayDate = date('Y-m-d');
}
...
?>
                ...
                <div class="layout-content">
                    ...
                    <div class="container-fluid flex-grow-1 container-p-y">
                        <h4 class="font-weight-bold py-3 mb-0"><?php if($_GET['id']) {?>Edit <?php } else{?> Pay
                            <?php } ?> Vendor Amount</h4>
...
<label class="form-label"> Invoice No<span class="text-danger">*</span></label>
 <select class="select2-demo form-control" name="InvoiceNo" id="InvoiceNo" required>
<option selected="" value="">Select Invoice No</option>
 <?php 
  $sql12 = "SELECT * FROM tbl_request_product_stock WHERE FinancerStatus2=1 AND VedId='".$row7['UserId']."'";
  $row12 = getList($sql12);
  foreach($row12 as $result){
     ?>
  <option <?php if($row7['InvoiceNo'] == $result['id']) {?> selected <?php } ?> value="<?php echo $result['id'];?>">
    INV<?php echo $result['id']; ?></option>
<?php } ?>
</select>
<div class="clearfix"></div>
</div>
...
<input type="hidden" name="CustName" id="CustName" value="<?php echo $CustName;?>"> 
...
<div class="form-group col-md-12">
   <label class="form-label">Narration</label>
    ...
    <div class="clearfix"></div>
                            ...
                            <div class="col-lg-6" id="custresult">
    ...
    function getCollections(recid){
         ...
         var CustId = $('#CustId').val();
            $.ajax({
                url: "ajax_files/ajax_vendor_ledger.php",
                method: "POST",
                data: {
                    action: action,
                    id: recid,
...
//console.log(data);exit();
                        if (data == 0) {
                            ...
                            success_toast();
                            setTimeout(function() {
                                window.location.href = 'view-vendor-payment.php';
                            }, 2000);
                        }
            ...
            $.ajax({
                ...
                success: function(data) {
                   console.log(data);
                    var res = JSON.parse(data);
                   // $('#InvNo').val(res.InvNo); 
                    $('#PaidAmount').val(res.TotPaidAmt);
                    $('#TotalInvAmt').val(res.TotAmt);
                    $('#BalanceAmt').val(res.TotBalAmt);
                    getCollections(val);
