...
$InvoiceNo = "00".$MaxId;
 ...
 $sql = "INSERT INTO tbl_sell SET SrNo='$MaxId',CustId='$CustId',CustName='$CustName',CellNo='$CellNo',Address='$Address',InvoiceNo='$InvoiceNo',InvoiceDate='$InvoiceDate',PayType='$PayType',Narration='$Narration',ProdType='$ProdType',PayMode='$PayMode',DeliveryDate='$DeliveryDate',GrossAmt='$GrossAmt',CgstPer='$CgstPer',CgstAmt='$CgstAmt',SgstPer='$SgstPer',SgstAmt='$SgstAmt',IgstPer='$IgstPer',IgstAmt='$IgstAmt',SubTotal='$SubTotal',UcdAmt='$UcdAmt',Status=1,CreatedBy='$user_id',CreatedDate='$CreatedDate',Discount='$Discount',Total='$Total',ChequeNo='$ChequeNo',ChqDate='$ChqDate',BankName='$BankName',UpiNo='$UpiNo',CreatedTime='$CreatedTime',BranchId='$BranchId',SellType='Challan',WarrantyPeriod='$WarrantyPeriod',PayStatus='$PayStatus',LrNo='$LrNo',LrDate='$LrDate',Transport='$Transport'";
$conn->query($sql);
            ...
            if(trim($_POST["ProductId"][$i] != ''))  
              {
                ...
                if($Qty > 0){
                $sql22 = "INSERT INTO tbl_sell_products SET UserId='$CustId',SellId='$SellId',ProductName='$ProductName',Purity='$Purity',Weight='$Weight',Price='$Price',Making='$Making',HmCharge='$HmCharge',Qty='$Qty',TotalRate='$TotalRate',ProductId='$ProductId',ModelNo='$ModelNo',SellDate='$InvoiceDate',SerialNo='$SerialNo'";
                $conn->query($sql22);
            ...
            if(trim($_POST["SerialProd"][$i] != ''))  
              {
                ...
                $sql22 = "INSERT INTO tbl_sell_products SET UserId='$CustId',SellId='$SellId',ProductName='$ProductName',Purity='$Purity',Weight='$Weight',Price='$Price',Making='$Making',HmCharge='$HmCharge',Qty='1',TotalRate='$TotalRate',ProductId='$ProductId',ModelNo='$ModelNo',SellDate='$InvoiceDate',SerialNo='$SerialNo'";
                $conn->query($sql22);
                $PostId = mysqli_insert_id($conn);
    ...
    <tr>
       ...
       <!--  <th>Rate</th>
        ...
        <tbody id="dynamic_field" >
            <?php 
             $sql12 = "SELECT ts.*,tp.ProductName AS Product_Name,tp.ModelNo AS Model_No,tp.Unit FROM tbl_stocks ts LEFT JOIN tbl_products tp ON ts.ProductId=tp.id WHERE ts.CrDr='cr' AND ts.ProdType=0 ORDER BY ts.ProductId";
  $row12 = getList($sql12);
  foreach($row12 as $result){
    $sql11 = "SELECT SUM(Qty) AS CrQty FROM tbl_stocks WHERE CrDr='cr' AND ProductId='".$result['ProductId']."'";
    $row11 = getRecord($sql11);
    $CrQty = $row11['CrQty'];

    $sql12 = "SELECT SUM(Qty) AS DrQty FROM tbl_stocks WHERE CrDr='dr' AND ProductId='".$result['ProductId']."'";
...
<td><input type="number" name="Qty[]" id="Qty1" class="form-control" placeholder="e.g.,1" value="1" autocomplete="off" min="0"></td>
...
<div class="col-lg-12">
     <select class="select2-demo form-control" multiple style="width: 100%" name="SerialProd[]">
                                    <?php 
                                        $sql22 = "SELECT * FROM tbl_stocks WHERE CrDr='cr' AND ProdType='1'";
                                        $row22 = getList($sql22);
