...
<head>
<title><?php echo $Proj_Title; ?> | FAQ's</title>
<meta charset="utf-8">
...
<h4 class="font-weight-bold py-3 mb-0">List Of FAQ's
...
<span class="font-weight-light">FAQ's</span>
</h5>
...
<textarea name="Answer" class="form-control" placeholder="Answer" id="Answer" autocomplete="off" rows="5" required></textarea>
<div class="clearfix"></div>
</div>
</div>
...
function update_toast(){
  ...
  $(document).ready(function() {
    ...
    $('#example').DataTable( {
      ...
      $('#add_button').click(function(){  
           $('.modal-title').html("Add <span class='font-weight-light'>FAQ's</span>");  
           $('#action').val("Add");  
           $('#id').val('');
                ...
                success:function(data){  
                    ...
                    $('#Status').val(data.Status).attr("selected",true);  
                     ...
                     $('#action').val('Edit'); 
                       ...
                       $('#submit').text("Update");   
                       $('.insert_frm').modal('show');
                         $('.modal-title').html("Update <span class='font-weight-light'>FAQ's</span>"); 
 ...
 //alert(id);
   swal({
            title: "Are you sure?",
            text: "You will not be able to recover this FAQ's!",
            type: "warning",
