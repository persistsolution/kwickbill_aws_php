...
<head>
<title><?php echo $Proj_Title; ?> | Payment Method</title>
<meta charset="utf-8">
...
<h4 class="font-weight-bold py-3 mb-0">List Of Payment Method
...
<input type="text" name="Name" class="form-control" id="Name" placeholder="Payment Method Name" required>
...
function category_lists(){
  var action = 'view';
      $.ajax({
  type: "POST",
  url: "ajax_files/ajax_payment_method.php",
   data:{action:action},  
  ...
  $(document).ready(function() {
    ...
    $('#example').DataTable( {
      ...
      $('#add_button').click(function(){  
           $('.modal-title').html("Add <span class='font-weight-light'>Payment Method</span>");  
           $('#action').val("Add");  
           $('#id').val('');
                ...
                success:function(data){  
                    ...
                    $('#Status').val(data.Status).attr("selected",true);  
                     ...
                     $('#action').val('Edit'); 
                       ...
                       $('#submit').text("Update");   
                       $('.insert_frm').modal('show');
                         $('.modal-title').html("Update <span class='font-weight-light'>Payment Method</span>"); 
 ...
 //alert(id);
   swal({
            ...
            if (isConfirm) {
                ...
                success:function(data){
              swal("Deleted!", "Payment Method has been deleted.", "success");
              ...
              category_lists();
                ...
                swal("Cancelled", "Payment Method is safe :)", "error");
            }
        });
