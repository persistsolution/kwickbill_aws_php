...
<?php } ?>
        ...
        <div class="main-container">
            <div class="container">
                 ...
                 <a href="add-vendor-expenses-2.php" class="btn btn-sm btn-default rounded">Add New</a>
                       ...
                       <?php 
$sql = "SELECT te.*, tub.ShopName, tu3.Fname AS VedName,tu3.TrustedVendor 
  ...
  <div class="card-body">
    <div class="row">
      ...
      <div class="col-md-9">
        ...
        <!-- Status Section -->
        <?php if($result['TrustedVendor'] == 'No'){?>
        <p class="mb-1">
...
<?php 
$bdm = $result['BdmStatus'];
$purchase = $result['PurchaseStatus'];
$manager = $result['ManagerStatus'];
$admin = $result['AdminStatus'];

$statuses = [$bdm, $purchase, $manager, $admin];

$hasApproved = in_array('1', $statuses); // someone approved
$hasRejected = in_array('2', $statuses); // someone rejected
$allPending = count(array_unique($statuses)) === 1 && $statuses[0] == '0'; // all pending

// Show button only if: at least one is rejected OR all are pending
if ($hasRejected || $allPending) {
?>
...
&nbsp;
        <!-- Delete Button -->
        <a href="view-vendor-expenses-2.php?action=delete&id=<?= $result['id']; ?>" 
