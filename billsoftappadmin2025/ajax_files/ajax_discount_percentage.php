...
$Status = $_POST["Status"];
$query = "SELECT * FROM tbl_billsoft_discount WHERE Percentage = '$Name'";
...
$qx = "INSERT INTO tbl_billsoft_discount SET Percentage = '$Name',modified_time='$modified_time',push_flag=1";
$conn->query($qx);
	echo 1;
}
}
...
if($_POST['action'] == 'fetch_record'){
 $id = $_POST['id'];
    $query = "SELECT * FROM tbl_billsoft_discount WHERE id = '$id'";
    $result = $conn->query($query);
...
$modified_time = gmdate('Y-m-d H:i:s.') . gettimeofday()['usec'];
$Status = $_POST["Status"];
$query = "SELECT * FROM tbl_billsoft_discount WHERE Percentage = '$Name' AND id != '$id'";
$result = $conn->query($query);
$row_cnt = mysqli_num_rows($result);
if($row_cnt > 0){
  echo 0;
  ...
  $query2 = "UPDATE tbl_billsoft_discount SET Percentage = '$Name',modified_time='$modified_time',push_flag=1 WHERE id = '$id'";
 	$conn->query($query2);
  echo 1;
}
}
  ...
  if($_POST['action'] == 'delete') {
      ...
      $id = $_POST['id'];
      $modified_time = gmdate('Y-m-d H:i:s.') . gettimeofday()['usec'];
      $query = "UPDATE tbl_billsoft_discount SET delete_flag=1,modified_time='$modified_time',push_flag=1 WHERE id = '$id'";
      $conn->query($query);
...
<table id="example" class="table table-striped table-bordered dt-responsive nowrap" style="width:100%">
        ...
        <tbody>
          <?php 
 $srno = 1;
  $sql = "SELECT * FROM tbl_billsoft_discount ORDER BY id DESC";
   $rx = $conn->query($sql);
