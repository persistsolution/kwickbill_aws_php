...
try {
    $ToFrId = $_REQUEST['user_id'] ?? '';

    if (empty($ToFrId)) {
        throw new Exception("ToFrId is required");
    }

    $sql = "SELECT tp.*, tu.ShopName AS FromFranchise, tu2.ShopName AS ToFranchise 
    ...
    while ($row = $res->fetch_assoc()) {
        ...
        $sql22 = "SELECT COUNT(*) AS cnt FROM tbl_transfer_franchise_prod_stock_items_2025 WHERE TransferId = '$transferId' AND Receive = 1";
