...
$id = $_GET['id'];
$sql7 = "SELECT * FROM tbl_attendance_request WHERE id='$id'";
...
?>
   ...
   <div class="card-body">
                               ...
                               <select class="form-control" name="AttRoll" id="AttRoll" required onchange="updateOutDate(this.value,document.getElementById('FromDate').value)">

 

<option value="0" <?php if($row7["AttRoll"] == 0) {?> selected <?php } ?>>
   Day</option>
   <option value="1" <?php if($row7["AttRoll"] == 1) {?> selected <?php } ?>>
   Night</option>
                                        ...
                                        <div class="form-group col-md-2">
                                            ...
                                            <input type="date" name="FromDate" id="FromDate" class="form-control"
                                                ...
                                                autocomplete="off" required onchange="updateOutDate(document.getElementById('AttRoll').value,this.value)">
                                            <div class="clearfix"></div>
                                        </div>
...
<?php include 'footer.php';?>
    ...
    <script>
    function updateOutDate(type,outdate){
        if(type == 0){
           $('#ToDate').val(outdate).attr("readonly", true);
        }
        else{
            $('#ToDate').val(outdate).attr("readonly",false);
        }
    }
     ...
     $.ajax({  
                url :"ajax_files/ajax_attendance_request.php",  
                method:"POST",  
...
swal({
  title: "Thank you",
  text: "Your attendance request successfully submitted.",
  type: "success",
  confirmButtonText: "OK"
},
function(isConfirm){
  if (isConfirm) {
          window.location.href="view-attendance-request.php";
  }
}); });
...
swal({
  title: "Error",
  text: "You have already submitted 5 or more requests this month.",
  type: "error",
  confirmButtonText: "OK"
},
function(isConfirm){
