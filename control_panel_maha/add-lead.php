...
<label class="form-label"> Employee/Users </label>
 <select class="select2-demo form-control" name="CustId" id="CustId" required>
     <option selected="" value="">Select </option>
  <optgroup label="Executive/Service Manager">    
 <?php 
...
<optgroup label="Sales Lead Manager">
     <?php 
...
<optgroup label="Flexi Manager">
     <?php 
  $sql12 = "SELECT * FROM tbl_users WHERE Status='1' AND Roll IN(20)";
  $row12 = getList($sql12);
  foreach($row12 as $result){
     ?>
  <option <?php if($row7["CustId"] == $result['id']) {?> selected <?php } ?> value="<?php echo $result['id'];?>">
