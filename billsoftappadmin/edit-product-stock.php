...
<?php 
$id = $_GET['id'];
$sql77 = "SELECT * FROM tbl_request_product_stock_2025 WHERE id='".$_GET['id']."'";
$row77 = getRecord($sql77);
$FromFrId = $row77['FromFrId'];
...
if(isset($_POST['submit'])){
    ...
    $sql = "UPDATE tbl_request_product_stock_2025 SET TotQty='$TotQty',TotalAmount='$TotalAmount',GstAmount='$GstAmount',UpdatedDate='$StockDate',Remark='$Remark',UpdatedBy='$user_id',UpdatedDateTime='$CreatedDate',UpdateStatus=1,Status=1,VedId='$VedId' WHERE id='$id'";
    $conn->query($sql);
    
     $sql = "INSERT INTO tbl_transfer_godown_raw_prod_stock_2025 SET GodownId='$GodownId',FranchiseId='$FranchiseId',StockDate='$StockDate',TotQty='$TotQty',Narration='$Remark',CreatedBy='$user_id',CreatedDate='$CreatedDate',TotalAmount='$TotalAmount',InvoiceNo='$InvoiceNo',GstAmount='$GstAmount',OwnShop=1";
    $conn->query($sql);
                    ...
                    if ($number > 0) {
                        for ($i = 0; $i < $number; $i++) {
                            if (trim($_POST["ProdId"][$i] != '')) {
                                ...
                                $MainProdId = addslashes(trim($_POST['MainProdId'][$i]));
                                ...
                                $conn->query($sql33);
                                 ...
                                 $sql33 = "INSERT INTO tbl_transfer_godown_raw_stock_items_2025 SET GodownId='$GodownId',FranchiseId='$FranchiseId',ProdId='$MainProdId',FrProdId='$ProdId',Qty='$Qty',
                                 Unit='$Unit',CreatedDate='$CreatedDate',TransferId='$UseRawId',CreatedBy='$user_id',StockDate='$StockDate',Price='$Rate',TotalPrice='$Total',CgstPer='$CgstPer',
                                 SgstPer='$SgstPer',IgstPer='$IgstPer',CgstAmt='$CgstAmt',SgstAmt='$SgstAmt',IgstAmt='$IgstAmt',GstAmt='$GstAmt',OwnShop=1";
                                $conn->query($sql33);
                                
                                $sql22 = "INSERT INTO tbl_godown_raw_prod_stock_2025 SET GodownId='$GodownId',FranchiseId='$FranchiseId',ProdId='$ProdId',Qty='$Qty',Unit='$Unit',
                                CreatedDate='$CreatedDate',TransferId='$UseRawId',Status='Dr',Narration='$Narration',StockDate='$StockDate',CreatedBy='$user_id',Price='$Rate',
                                TotalPrice='$Total',CgstPer='$CgstPer',SgstPer='$SgstPer',IgstPer='$IgstPer',CgstAmt='$CgstAmt',SgstAmt='$SgstAmt',IgstAmt='$IgstAmt',GstAmt='$GstAmt',OwnShop=1";
                                $conn->query($sql22);
...
<?php 
  echo $sql12 = "SELECT * FROM tbl_users_bill WHERE id='$FromFrId' AND Roll=5";
  $row12 = getList($sql12);
  foreach($row12 as $result){
     ?>
 ...
 <div id="dynamic_field">
    <?php 
        ...
        foreach($row as $result){
             ...
             $sql12 = "SELECT id,ProdId,ProductName FROM tbl_cust_products_2025 WHERE id='".$result['ProdId']."'";
             $row2 = getRecord($sql12);
...
<label class="form-label">Product</label>
 <select class="form-control" name="ProdId[]" id="ProdId<?php echo $result['id'];?>"  onchange="getAvailProdStock(this.value,document.getElementById('srno1').value)">
  ...
  <option value="<?php echo $row2['id'];?>">
    <?php echo $row2['ProductName']; ?></option>

</select>
</div>

<input type="hidden" id="MainProdId<?php echo $result['id'];?>" name="MainProdId[]" value="<?php echo $row2['ProdId'];?>">
...
<input type="text" name="GstAmount" id="GstAmount" class="form-control" placeholder="" value="<?php echo number_format(($row77["TotalAmount"]*(5/100)),2); ?>" autocomplete="off" readonly>
<div class="clearfix"></div>
    </div>
...
<?php include_once 'footer_script.php'; ?>
 ...
 function getTotAmt(){
     ...
     $('#TotalAmount').val(sum);
     var GstAmt = Number(sum)*(Number(5)/Number(100));
     $('#GstAmount').val(parseFloat(GstAmt).toFixed(2));
    }
