...
if ($_POST['action'] == 'getRecordDetails') {
    $id = $_POST['uid'];
    $sql2 = "SELECT Amount FROM tbl_general_ledger WHERE UserId='$id' AND Type='OB'";
    $row2 = getRecord($sql2);
    $sql = "SELECT SUM(Amount) AS PaidAmt FROM tbl_general_ledger WHERE UserId='$id' AND Type!='OB' AND CrDr='Dr'";
    $row = getRecord($sql);
    $BalAmt = $row2['Amount'] - $row['PaidAmt'];
    // $sql = "SELECT SUM(debitqty) AS SumDebit,SUM(creditqty) AS SumCredit, SUM(debitqty)-sum(creditqty) AS BalAmt FROM 
    //             (SELECT (case when CrDr='Dr' then sum(Amount) else '0' end) as debitqty,(case when CrDr='Cr' then sum(Amount) else '0' end) as creditqty
    //             FROM `tbl_general_ledger` WHERE UserId='$id' AND Type!='OB' GROUP by CrDr) as a";
    // $row = getRecord($sql);
...
if ($_POST['action'] == 'getCollections') {
    ...
    <table id="example" class="table table-striped table-bordered" style="width:100%">
        ...
        <tbody>
            <?php
            $srno = 1;
            $sql = "SELECT * FROM tbl_general_ledger WHERE UserId='$id' AND Type!='PW' ORDER BY PaymentDate";
            $rx = $conn->query($sql);
...
if($_POST['action'] == 'SaveDailyCollection'){
	$id = $_POST['id'];
...
if($id==''){
	...
	$sql = "INSERT INTO tbl_general_ledger SET SrNo='$SrNo',Code='$Code',InvId='$InvId',InvNo='$InvNo',CustId='$CustId',CustName='$CustName'
,Amount='$Amount',PaymentDate='$PayDate',PayMode='$PayType',ChequeNo='$ChequeNo',ChqDate='$ChqDate',BankName='$BankName',UpiNo='$UpiNo',
Narration='$Narration',Type='PR',CrDr='cr',CreatedBy='$user_id',CreatedDate='$CreatedDate',Main=0";
$conn->query($sql);
	...
	$sql4 = "UPDATE tbl_general_ledger SET InvId='$InvId',InvNo='$InvNo',CustId='$CustId',CustName='$CustName'
,Amount='$Amount',PaymentDate='$PayDate',PayMode='$PayType',ChequeNo='$ChequeNo',ChqDate='$ChqDate',BankName='$BankName',UpiNo='$UpiNo',
Narration='$Narration',Type='PR',CrDr='cr',ModifiedBy='$user_id',ModifiedDate='$CreatedDate' WHERE id='$id'";
    $conn->query($sql4);
	echo 1;
	}
}
