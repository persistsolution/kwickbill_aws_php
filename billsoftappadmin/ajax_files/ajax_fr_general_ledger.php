...
if ($_POST['action'] == 'getInvoice') {
    ...
    <?php
    $sql1 = "SELECT * FROM tbl_customer_invoice WHERE CustId='$id' AND Roll=1";
    $row1 = getList($sql1);
...
if ($_POST['action'] == 'getInvoiceDetails') {
    $id = $_POST['id'];
    $CustId = $_POST['CustId'];
    $sql = "SELECT NetAmount AS Amount FROM tbl_customer_invoice WHERE InvoiceNo='$id' AND Roll=1";
    $row = getRecord($sql);
    $sql2 = "SELECT SUM(Amount) As TotPaidAmt FROM tbl_general_ledger WHERE InvoiceNo='$id' AND Type='PR' AND CrDr='cr' AND UserId='$CustId' AND Flag='Franchise-Invoice' AND AccRoll='Franchise'";
    $row2 = getRecord($sql2);
    $TotAmt = $row['Amount'];
...
if ($_POST['action'] == 'getCustBalAmt') {
    $uid = $_POST['uid'];
    //paid amt
    $sql2 = "SELECT SUM(Amount) As TotPaidAmt FROM tbl_general_ledger WHERE UserId='$uid' AND CrDr='cr' AND AccRoll='Franchise'";
    $row2 = getRecord($sql2);
    $TotPaidAmt = $row2['TotPaidAmt'];

    //debit amt
    $sql2 = "SELECT SUM(Amount) As TotAmt FROM tbl_general_ledger WHERE UserId='$uid' AND CrDr='dr' AND AccRoll='Franchise'";
    $row2 = getRecord($sql2);
    $TotAmt = $row2['TotAmt'];
    $TotBalAmt = $TotAmt - $TotPaidAmt;
    echo json_encode(array('TotAmt' => $TotAmt, 'TotPaidAmt' => $TotPaidAmt, 'TotBalAmt' => $TotBalAmt));
...
if ($_POST['action'] == 'getCollections') {
    ...
    <table id="example" class="table table-striped table-bordered" style="width:100%">
        ...
        <tbody>
            <?php
            $srno = 1;
            $sql = "SELECT * FROM tbl_general_ledger WHERE UserId='$CustId' AND InvoiceNo='$id' AND Type IN ('PR','OB') AND Flag='Franchise-Invoice' AND AccRoll='Franchise' ORDER BY id";
            $rx = $conn->query($sql);
...
$sql4 = "INSERT INTO tbl_general_ledger SET SrNo='$SrNo',Code='$Code',UserId='$CustId',
                            ...
                            SellId='$SellId',Flag='Franchise-Invoice',AccRoll='Franchise',CreatedBy='$user_id'";
                        $conn->query($sql4);
                        ...
                        $sql4 = "INSERT INTO tbl_general_ledger SET SrNo='$SrNo',Code='$Code',UserId='0',
                            ...
                            Type='PR',CreatedDate='$CreatedDate',PaymentDate='$PayDate',
                            PayMode='$PayType',ChequeNo='$ChequeNo',ChqDate='$ChqDate',BankName='$BankName',UpiNo='$UpiNo',Narration='$Narration',
                            SellId='$SellId',Flag='Franchise-Invoice',AccRoll='Company',PostId='$PostId',CreatedBy='$user_id'";
                        $conn->query($sql4);
