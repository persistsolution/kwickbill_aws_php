...
$sql = "SELECT  SUM(NetAmount) AS NetAmount FROM tbl_customer_invoice_2025 WHERE Roll=2 AND Status=1 AND FrId='$BillSoftFrId' AND delete_flag=0 AND Discount>0";
...
<?php 
    function calAmt($type,$BillSoftFrId){
        global $conn;
         $sql = "SELECT SUM(NetAmount) AS NetAmount FROM tbl_customer_invoice_2025 WHERE Roll=2 AND Status=1 AND FrId='$BillSoftFrId' AND PayType='$type' AND delete_flag=0 AND Discount>0";
         if($_POST['ReportType'] !=''){
                $ReportType = $_POST['ReportType'];
                if($ReportType == 'Today'){
                    $sql.=" AND InvoiceDate='".date('Y-m-d')."'";
...
<table id="example" class="table mb-0 dataTable no-footer" role="grid" aria-describedby="report-table_info">
        ...
        <tbody>
            <?php 
            $i=1;
            $sql = "SELECT * FROM tbl_customer_invoice_2025 WHERE Roll=2 AND Status=1 AND  delete_flag=0 AND Discount>0 AND FrId!=0";
           ...
           // echo $sql;
            ...
            <tr>
               ...
               <td class="align-middle"><?php echo $row3['ShopName'];?></td>
              ...
              <td class="align-middle"><?php echo date("d/m/Y", strtotime(str_replace('-', '/',$row['InvoiceDate']))); ?></td>
                ...
                <td class="align-middle"><?php echo $row['CustName']; ?></td>
                 <td class="align-middle"><?php echo $row['CellNo']; ?></td>
           ...
           <tr>
               ...
               <th><?php echo number_format(($TotDiscount/$TotSubTotal)*100,2);?>%</th>
