...
{
  $id = $_REQUEST["id"];
  $sql11 = "DELETE FROM tbl_nso_vendor_expenses WHERE id = '$id'";
  $conn->query($sql11);
  ?>
...
<?php } ?>
        ...
        <div class="main-container">
            <div class="container">
                 ...
                 <a href="add-nso-vendor-expenses.php" class="btn btn-sm btn-default rounded">Add New</a>
  ...
  <div class="card-body">
    <div class="row">
      <div class="col-md-12">
        ...
        <!-- NSO Dept -->
        <p class="mb-1"><strong>NSO Dept Status:</strong>
          <?php
          if ($result['BdmStatus'] == '1') {
              echo "<span class='text-success'>Approved</span> | $bdmdate";
          } elseif ($result['BdmStatus'] == '2') {
              echo "<span class='text-danger'>Rejected</span> | $bdmdate<br><strong>Comment:</strong> " . nl2br($result['BdmComment']);
          } else {
        ...
        <!-- Purchase Dept -->
        <p class="mb-1"><strong>Purchase Dept Status:</strong>
          <?php
          if ($result['PurchaseStatus'] == '1') {
              echo "<span class='text-success'>Approved</span> | $purchasedate";
          } elseif ($result['PurchaseStatus'] == '2') {
              echo "<span class='text-danger'>Rejected</span> | $purchasedate<br><strong>Comment:</strong> " . nl2br($result['PurchaseComment']);
          } else {
              echo "<span class='text-warning'>Pending</span>";
          }
          ?>
        ...
        <!-- Accountant -->
        <p class="mb-1"><strong>Accountant Status:</strong>
          <?php
          if ($result['ManagerStatus'] == '1') {
              echo "<span class='text-success'>Approved</span> | $accountdate";
          } elseif ($result['ManagerStatus'] == '2') {
              echo "<span class='text-danger'>Rejected</span> | $accountdate<br><strong>Comment:</strong> " . nl2br($result['MannagerComment']);
          } else {
              echo "<span class='text-warning'>Pending</span>";
          }
          ?>
        ...
        <p class="mb-0"><strong>Admin Status:</strong>
          ...
          if ($result['AdminStatus'] == '1') {
              ...
              echo "<span class='text-danger'>Rejected</span> | $admindate<br><strong>Comment:</strong> " . nl2br($result['AdminComment']);
          } else {
              echo "<span class='text-warning'>Pending</span>";
          }
          ?>
      ...
      <?php if($result['BdmStatus']==1 || $result['PurchaseStatus']==1 || $result['ManagerStatus']==1 || $result['AdminStatus']==1){} else{?>
      <div class="col-md-3 d-flex align-items-start justify-content-end">
        <div class="d-flex gap-2">
          <!-- Edit Button -->
          <a href="add-nso-vendor-expenses.php?id=<?= $result['id']; ?>" 
...
&nbsp;
          <!-- Delete Button -->
          <a href="view-nso-vendor-expenses.php?action=delete&id=<?= $result['id']; ?>" 
