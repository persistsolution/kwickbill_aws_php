...
$Calendar = $_REQUEST['calendar'];
 $sql = "SELECT * FROM tbl_cust_category WHERE CreatedBy IN (0,'$FranchiseId') ORDER BY srno asc";
 $row = getList($sql);
 foreach($row as $result){
     $sql2 = "SELECT SUM(tcid.Total) AS Total,COUNT(tcid.id) AS TotProd FROM tbl_customer_invoice_details tcid INNER JOIN tbl_customer_invoice tci ON tci.id=tcid.InvId WHERE tcid.CatId='".$result['id']."' AND tci.Roll=2 AND tci.FrId='$FranchiseId'";
     if($Calendar == 'yesterday'){
            ...
            $sql2.=" AND tci.InvoiceDate>='$Week' AND tci.InvoiceDate<='".date('Y-m-d')."'";
        }
                            ...
                            <li class="list-group-item" style="border-radius: 10px;">
                                <div class="row align-items-center">
                                    <div class="col align-self-center pr-0">
                                        ...
                                        <p class="small text-secondary">Total Items : <?php echo $row2['TotProd'];?></p>
                                    </div>
                                    <div class="col-auto">
                                       ₹<?php echo number_format($row2['Total'],2);?>
                                    </div>
                                </div>
                            </li>
