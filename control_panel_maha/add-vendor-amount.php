...
$id = $_GET['id'];
$recid = $_GET['recid'];
$sql7 = "SELECT * FROM tbl_vendor_expense_ledger WHERE id='$id'";
$row7 = getRecord($sql7);
if($_GET['id'] == ''){
    $PayDate = date('Y-m-d');
}
...
<label class="form-label"> Invoice No<span class="text-danger">*</span></label>
 <select class="select2-demo form-control" name="InvId" id="InvId" required>
<option selected="" value="">Select Invoice No</option>
 <?php 
  $sql12 = "SELECT InvoiceNo,ExpId FROM tbl_vendor_expense_ledger WHERE Roll='3' AND Type='INV' AND CrDr='cr' AND UserId='".$row7['UserId']."'";
  $row12 = getList($sql12);
  foreach($row12 as $result){
     ?>
  <option <?php if($row7['InvId'] == $result['ExpId']) {?> selected <?php } ?> value="<?php echo $result['ExpId'];?>">
...
<div class="clearfix"></div>
 ...
 <input type="hidden" name="InvNo" id="InvNo" value="<?php echo $row7['InvoiceNo'];?>">
...
<div class="form-group col-md-12">
   <label class="form-label">Narration</label>
    ...
    function getVedInvoice(vedid){
        var action = "getVedInvoice";
            $.ajax({
                url: "ajax_files/ajax_vendor_account.php",
                method: "POST",
                data: {
                    action: action,
                    id: vedid
