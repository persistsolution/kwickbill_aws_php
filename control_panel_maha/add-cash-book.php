...
<?php 
  if(isset($_POST['submit'])){
    ...
    if($_GET['id'] == ''){
        ...
        $rncnt = getRow($sql);
        if($rncnt > 0){
            echo "<script>alert('Same Date & Same Amount Cash Book Record Already Exists!');window.location.href='add-cash-book.php';</script>";
        }
        else{
  ...
  echo "<script>alert('Record Saved Successfully!');window.location.href='view-cash-book.php';</script>";
    ...
    $sql = "UPDATE tbl_cash_book SET FrId='$BillSoftFrId',FromDate='$FromDate',ToDate='$ToDate',TotalAmount='$TotalAmount',Amount='$Amount',
            BalAmt='$BalAmt',TransferDate='$TransferDate',Narration='$Narration',BankName='$BankName',AccountNo='$AccountNo',Files='$Files',ModifiedBy='$user_id',ModifiedDate='$CreatedDate' WHERE id='$id'";
    $conn->query($sql);
...
<label class="form-label"> Franchise<span class="text-danger">*</span></label>
 <select class="select2-demo form-control" name="UserId" id="UserId" required onchange="getTotalCashAmt(this.value)">
...
<input type="date" name="FromDate" id="FromDate" class="form-control" placeholder="" value="<?php echo $row7["FromDate"]; ?>" autocomplete="off" required onchange="getCashAmount(document.getElementById('FromDate').value,document.getElementById('ToDate').value,<?php echo $BillSoftFrId;?>)">
<div class="clearfix"></div>
    </div>
    
    <div class="form-group col-md-6">
<label class="form-label">To Date <span class="text-danger">*</span></label>
<input type="date" name="ToDate" id="ToDate" class="form-control" placeholder="" value="<?php echo $row7["ToDate"]; ?>" autocomplete="off" required onchange="getCashAmount(document.getElementById('FromDate').value,document.getElementById('ToDate').value,<?php echo $BillSoftFrId;?>)">
<div class="clearfix"></div>
    </div>-->
...
<input type="text" name="AccountNo" id="AccountNo" class="form-control" placeholder="" value="<?php echo $row7["AccountNo"]; ?>" autocomplete="off" required>
<div class="clearfix"></div>
    </div>
...
<?php include_once 'footer_script.php'; ?>
 ...
 function balAmt(){
  ...
  function getTotalCashAmt(frid){
      var action = "getTotalCashAmt";
            $.ajax({
                url: "ajax_files/ajax_customer_account.php",
                method: "POST",
                data: {
