...
$FranchiseId = $_SESSION['FranchiseId'];
  if(isset($_POST['submit'])){
    ...
    if($_GET['id'] == ''){
     $qx = "INSERT INTO tbl_cust_prod_stock SET ProdId='$ProdId',Qty='$Qty',CreatedBy='$user_id',StockDate='$StockDate',Narration='$Narration',Status='Cr',UserId='$FranchiseId',CreatedDate='$CreatedDate',FrId='$FranchiseId'";
  $conn->query($qx);
  ...
  echo "<script>alert('Product Stock Added Successfully!');window.location.href='view-cust-stocks.php';</script>";
    ...
    $sql = "UPDATE tbl_cust_prod_stock SET ProdId='$ProdId',Qty='$Qty',ModifiedBy='$user_id',StockDate='$StockDate',Narration='$Narration',Status='Cr',ModifiedDate='$CreatedDate' WHERE id='$id'";
    $conn->query($sql);
...
<?php 
  $sql12 = "SELECT * FROM tbl_cust_products WHERE Status='1' AND CreatedBy='$FranchiseId' ORDER BY ProductName";
  $row12 = getList($sql12);
