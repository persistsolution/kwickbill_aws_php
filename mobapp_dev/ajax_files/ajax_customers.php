...
$sql = "INSERT INTO tbl_users SET Fname='$Fname',Lname='$Lname',Phone='$Phone',EmailId='$EmailId',Password='$Password',CountryId='$CountryId',StateId='$StateId',CityId='$CityId',AreaId='$AreaId',Address='$Address',Pincode='$Pincode',Status='1',Roll='$Roll',CreatedDate='$CreatedDate',KycStatus=0,ReferCode='$ReferCode'";
$conn->query($sql);
...
$smstxt = "Please enter ".$otp." OTP on our platform to complete the verification process. Thank you for choosing Maha Chai.";
 ...
 include '../../incsmsapi.php';  
     ...
     echo json_encode(array('msg'=>"Registration Successfull!...",'status'=>'1','Username'=>$Phone,'uid'=>$CustId,'roll'=>$Roll));
}
}
...
$query = "SELECT * FROM tbl_users WHERE Phone = '$Username' AND Status=1";
if($Roll==5){
   $query.=" AND Roll='5'"; 
}
if($Roll==55){
...
if($Roll==6){
   $query.=" AND Roll NOT IN(1,5,55,9,22,23,63,3)"; 
}
if($Roll==9){
   $query.=" AND Roll IN(9,22,23)";  
}
if($Roll==3){
   $query.=" AND Roll='3'"; 
}
...
$query = "SELECT * FROM tbl_users WHERE Phone = '$Username' AND Password='$Password' AND Status=1";
if($Roll==5){
   $query.=" AND Roll='5'"; 
}
if($Roll==55){
...
if($Roll==6){
   $query.=" AND Roll NOT IN(1,5,55,9,22,23,63)"; 
}
if($Roll==9){
   $query.=" AND Roll IN(9,22,23)";  
}
...
if($Roll == 9){
 ...
 if($Roll == 55){
     $smstxt = "Hello ".$Fname." Thank you for registering with Maha Chai! We're delighted to welcome you to our community. Best regards, Maha Chai Team";
  $dltentityid = "1501701120000037351";
  $dlttempid = "1707169876321346109";
  include '../../incsmsapi.php';  
 }
echo json_encode(array('status'=>1,'Username'=>$Phone,'uid'=>$uid,'roll'=>$row['Roll']));
}
}
else{
echo json_encode(array('status'=>0));   
...
if($_POST['action']=='SaveKyc'){
  ...
  $CreatedDate = date('Y-m-d');
  $sql = "UPDATE tbl_users SET Profession='$Profession',PanNo='$PanNo',AadharNo='$AadharNo',FsiicNo='$FsiicNo',ShopActNo='$ShopActNo',Dob='$Dob',AnniversaryDate='$AnniversaryDate' WHERE id='$user_id'";
  $conn->query($sql);
  echo 1;
}
