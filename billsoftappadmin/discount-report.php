...
$sql = "SELECT  SUM(NetAmount) AS NetAmount FROM tbl_customer_invoice WHERE Roll=2 AND Status=1 AND FrId='$BillSoftFrId' AND delete_flag=0 AND Discount>0";
...
<?php 
    function calAmt($type,$BillSoftFrId){
        global $conn;
         $sql = "SELECT SUM(NetAmount) AS NetAmount FROM tbl_customer_invoice WHERE Roll=2 AND Status=1 AND FrId='$BillSoftFrId' AND PayType='$type' AND delete_flag=0 AND Discount>0";
         if($_POST['ReportType'] !=''){
                $ReportType = $_POST['ReportType'];
                if($ReportType == 'Today'){
                    $sql.=" AND InvoiceDate='".date('Y-m-d')."'";
...
<table id="example" class="table mb-0 dataTable no-footer" role="grid" aria-describedby="report-table_info">
        ...
        <tbody>
            <?php 
            $i=1;
            $sql = "SELECT * FROM tbl_customer_invoice WHERE Roll=2 AND Status=1 AND  delete_flag=0 AND Discount>0 AND FrId!=0";
