...
$query = "SELECT * FROM tbl_godown_prod_category WHERE Name = '$Name'";
...
if($row_cnt > 0){
 ...
 $qx = "INSERT INTO tbl_godown_prod_category SET Name = '$Name', Icon = '$Icon',Status='$Status',Featured='$Featured',Photo='$Photo',Photo2='$Photo2',CreatedDate='$CreatedDate',Roll='$Roll',srno='$srno'";
  $conn->query($qx);
  echo 1;
}
}
...
if($_POST['action'] == 'fetch_record'){
 $id = $_POST['id'];
    $query = "SELECT * FROM tbl_godown_prod_category WHERE id = '$id'";
    $result = $conn->query($query);
...
$query = "SELECT * FROM tbl_godown_prod_category WHERE Name = '$Name' AND id != '$id'";
$result = $conn->query($query);
$row_cnt = mysqli_num_rows($result);
if($row_cnt > 0){
  echo 0;
  ...
  $query2 = "UPDATE tbl_godown_prod_category SET Name = '$Name',Icon = '$Icon',Status='$Status',Featured='$Featured',Photo='$Photo',Photo2='$Photo2',ModifiedDate='$ModifiedDate',Roll='$Roll',srno='$srno' WHERE id = '$id'";
  $conn->query($query2);
  echo 1;
}
}
  ...
  if($_POST['action'] == 'delete') {
      ...
      $id = $_POST['id'];
       $query2 = "SELECT Photo FROM tbl_godown_prod_category WHERE id = '$id'";
    $result2 = $conn->query($query2);
    $row2 = $result2->fetch_assoc();
    $Photo = $row2['Photo'];
     $Photo2 = $row2['Photo2'];
      $query = "DELETE FROM tbl_godown_prod_category WHERE id = '$id'";
      $conn->query($query);
...
if($_POST['action'] == 'deletePhoto'){
    $id = $_POST['id'];
    $Photo = $_POST['Photo'];
        $q = "UPDATE tbl_godown_prod_category SET Photo='' WHERE id=$id";
        $conn->query($q);
        $src = "../../uploads/$Photo";
        unlink($src);
...
if($_POST['action'] == 'deletePhoto2'){
    $id = $_POST['id'];
    $Photo = $_POST['Photo'];
        $q = "UPDATE tbl_godown_prod_category SET Photo2='' WHERE id=$id";
        $conn->query($q);
        $src = "../../uploads/$Photo";
        unlink($src);
...
<table id="example" class="table table-striped table-bordered dt-responsive nowrap" style="width:100%">
        ...
        <tbody>
          <?php 
 $srno = 1;
  $sql = "SELECT * FROM tbl_godown_prod_category ORDER BY srno asc";
   $rx = $conn->query($sql);
