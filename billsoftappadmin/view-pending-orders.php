...
<table id="example" class="table mb-0 dataTable no-footer" role="grid" aria-describedby="report-table_info">
        ...
        <tbody>
            ...
            $sql = "SELECT * FROM tbl_customer_invoice WHERE Roll=2 AND Status=1";
            if($Roll == 1){
                $sql.=" AND FrId=0";
            }
            else{
             ...
             $sql = "SELECT * FROM tbl_customer_invoice WHERE Roll=2 AND Status=1 AND InvoiceDate='".date('Y-m-d')."'";
             if($Roll == 1){
                $sql.=" AND FrId=0";
            }
            else{
