...
<head>
<title><?php echo $Proj_Title; ?> | Attribute Name</title>
<meta charset="utf-8">
...
<h4 class="font-weight-bold py-3 mb-0">List Of Attribute Name
...
<span class="font-weight-light">Attribute Name</span>
</h5>
...
function category_lists(){
  var action = 'view';
      $.ajax({
  type: "POST",
  url: "ajax_files/ajax_attribute_name.php",
   data:{action:action},  
  ...
  function error_toast(){
   ...
   $.growl.success({
      title:    'Success',
      message:  'New Attribute Name Added Successfully!',
      location: isRtl ? 'tl' : 'tr'
    });
  }
function update_toast(){
  ...
  $(document).ready(function() {
    ...
    $('#example').DataTable( {
      ...
      $('#add_button').click(function(){  
           $('.modal-title').html("Add <span class='font-weight-light'>Attribute Name</span>");  
           $('#action').val("Add");  
           $('#id').val('');
                ...
                success:function(data){  
                    ...
                    $('#Status').val(data.Status).attr("selected",true); 
                     ...
                     $('#action').val('Edit'); 
                       ...
                       $('#submit').text("Update");   
                       $('.insert_frm').modal('show');
                         $('.modal-title').html("Update <span class='font-weight-light'>Attribute Name</span>"); 
 ...
 //alert(id);
   swal({
            title: "Are you sure?",
            text: "You will not be able to recover this Attribute Name!",
            type: "warning",
            ...
            if (isConfirm) {
                ...
                success:function(data){
              swal("Deleted!", "Attribute Name has been deleted.", "success");
              ...
              category_lists();
                ...
                swal("Cancelled", "Attribute Name is safe :)", "error");
            }
        });
